...
/* cz277 - xform */
/*void UpdateSpkrStatsLocal(HMMSet *hset, XFInfo *xfinfo, char *datafn) {
    char paspkr[MAXSTRLEN], newFn[MAXSTRLEN], newMn[MAXSTRLEN];
    Boolean maskMatch, spkrChange = FALSE;

    if (!(hset->hsKind == PLAINHS || hset->hsKind == SHAREDHS || hset->hsKind == HYBRIDHS))
        if (xfinfo->useOutXForm || xfinfo->useInXForm || xfinfo->usePaXForm)
            HError(9999, "UpdateSpkrStats: Adaptation only valid for PLAINHS, SHAREDHS, and HYBRIDHS systems");
    // First, handle output transform generation 
    if (xfinfo->usePaXForm == TRUE) {
        maskMatch = MaskMatch(xfinfo->paSpkrPat, paspkr, datafn);
        if (maskMatch == FALSE)
            HError(9999, "UpdateSpkrStats: Parent xform mask %s does not match file name %s", xfinfo->paSpkrPat, datafn);
        if (strcmp(paspkr, xfinfo->cpaspkr) != 0) {
    ...
    // Second, handle input transform 
    if (xfinfo->useInXForm == TRUE && datafn != NULL) {
