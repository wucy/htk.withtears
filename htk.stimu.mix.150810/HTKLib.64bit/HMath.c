...
void SplitGradDNNandMix(NMatrix *dest_dy, NMatrix *comb_dy, NMatrix *mix_surface, int batLen, int raw_dim, int comb_dim, float mixscaler) {
#ifdef CUDA
    NFloat *dest_dy_elems = dest_dy->devElems;
    NFloat *comb_dy_elems = comb_dy->devElems;
    NFloat *mix_surface_elems = mix_surface->devElems;
    SplitGradDNNandMixCUDA(dest_dy_elems, comb_dy_elems, mix_surface_elems, batLen, raw_dim, comb_dim, mixscaler);
#else
    HError(9999, "SplitGradDNNandMix: not support CPU.");
#endif
}

void ApplySigmoidActStimuMix(NMatrix *srcMat, int row, int col, NMatrix *dstMat, int mixnodes) {
#ifdef CUDA
    NFloat *srcMat_elems = srcMat->devElems;
    NFloat *dstMat_elems = dstMat->devElems;
    ApplySigmoidActStimuMixCUDA(srcMat_elems, row * col, dstMat_elems, mixnodes, col);
#else
