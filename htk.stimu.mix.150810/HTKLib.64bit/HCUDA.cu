...
__global__ void HKern_SplitGradDNNandMix_DNNpart(NFloat *dest_dy, NFloat *comb_dy, int batLen, int raw_dim, int comb_dim, float mixscaler, int tot) {
    ...
    dest_dy[bat_id * raw_dim + col_id] = (1 - mixscaler) * comb_dy[idx];
}
__global__ void HKern_SplitGradDNNandMix_Mixpart(NFloat *dest_dy, NFloat *comb_dy, NFloat *mix_surface, int batLen, int raw_dim, int comb_dim, int mixnodes, float mixscaler) {
    extern __shared__ NFloat tmpPtr[];
    ...
    if (thdIdx < thdNum && batIdx < batLen && mixIdx < mixnodes) {
        ...
        sumVal = tmpPtr[0];
        if (thdIdx == 0) {
            dest_dy[batIdx * raw_dim + comb_dim + mixIdx] = sumVal * mixscaler;
        }
    }
}

void SplitGradDNNandMixCUDA(NFloat *dest_dy, NFloat *comb_dy, NFloat *mix_surface, int batLen, int raw_dim, int comb_dim, float mixscaler) {    
    //THREADPERBLOCK
    int tot = comb_dim * batLen;
    int nBlocks = CEIL(tot, THREADPERBLOCK);
    HKern_SplitGradDNNandMix_DNNpart<<< nBlocks, THREADPERBLOCK >>>(dest_dy, comb_dy, batLen, raw_dim, comb_dim, mixscaler, tot);

    nBlocks = mixscaler * batLen;
    int sBytes = THREADPERBLOCK * sizeof(NFloat);
    HKern_SplitGradDNNandMix_Mixpart<<< nBlocks, THREADPERBLOCK, sBytes >>>(dest_dy, comb_dy, mix_surface, batLen, raw_dim, comb_dim, raw_dim - comb_dim, mixscaler);
}

__global__ void HKern_ApplySigmoidActStimuMix(NFloat *srcPtr, int len, NFloat *dstPtr, int mixnodes, int raw_col) {
    int pos;
    NFloat floatVal;
    pos = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (pos < len) {
        if (pos % raw_col  < raw_col - mixnodes) {
            floatVal = -1.0 * srcPtr[pos];
            CHKNFLTEXPE(floatVal)
            dstPtr[pos] = 1.0 / (1.0 + exp(floatVal));
        }
...
void ApplySigmoidActStimuMixCUDA(NFloat *srcMat, int len, NFloat *dstMat, int mixnodes, int raw_col) {
    ...
    HKern_ApplySigmoidActStimuMix<<<nBlocks, THREADPERBLOCK>>>(srcMat, len, dstMat, mixnodes, raw_col);
}
...
__global__ void HKern_FwdCombDNNandMix(NFloat *comb_yFeaMat, NFloat *mix_surface, NFloat *raw_yFeaMat, int batLen, int raw_nodeNum, int comb_nodeNum, int mixnodes, float mixscaler) {
    ...
    if (thdIdx == 0) {
        comb_yFeaMat[blockIdx.x] =   mixscaler * tmpPtr[0] + (1 - mixscaler) * raw_yFeaMat[my_comb_col + my_batid * raw_nodeNum];
    }
}

__global__ void HKern_CalcMixSoftmaxSum(NFloat *comb_softmaxSum, NFloat *raw_yFeaMat, int batLen, int raw_nodeNum, int comb_nodeNum, int mixnodes) {
    extern __shared__ NFloat tmpPtr[];
    int my_idx = threadIdx.x + blockIdx.x * blockDim.x;
...
void FwdCombDNNandMixCUDA(NFloat *comb_yFeaMat, NFloat *mix_surface, NFloat *raw_yFeaMat, NFloat *comb_softmaxSum, int batLen, int raw_nodeNum, int comb_nodeNum, int mixnodes, float mixscaler) {
    ...
    HKern_CalcMixSoftmaxSum<<< batLen, mixnodes, sBytes >>>(comb_softmaxSum, raw_yFeaMat, batLen, raw_nodeNum, comb_nodeNum, mixnodes);
