...
void DoRecognition (DecoderInst *dec, char *fn)
{
    ...
    if (pbInfo.tgtPK != hset.pkind)
        HError(9999, "HDecode: Incompatible parm kinds %s vs. %s", ParmKind2Str(pbInfo.tgtPK, buf1), ParmKind2Str(hset.pkind, buf2));
    ...
    if (hset.annSet == NULL) { /* use conventional way */
        ...
        while (TRUE) {
            ...
            for (i = 1; i <= hset.swidth[0]; ++i) {
                ...
                finish[i] = FillAllInpBatch(cache[i], &nLoaded, &uttCnt);
                /* cz277 - mtload */
            ...
            /* decode these frames */
            for (i = 0; i < nLoaded; ++i) {
                ProcessFrame(dec, obsBlock, nLoaded, xfInfo.inXForm, i);
            }
        ...
        if (frameN != uttLen) {
            HError(9999, "The number of frame loaded does not match the utterance length");
        }
        /* cz277 - mtload */
        for (i = 1; i <= hset.swidth[0]; ++i) {
    ...
    if (trans) {
        ...
        if (labOFileMask) {
            if (!MaskMatch (labOFileMask, lfn, fn))
                HError(2319,"HLRescore: LABOFILEMASK %s has no match with segemnt %s", labOFileMask, fn);
        } else {     
            strcpy (lfn, fn);
        }
        ...
        if (lat) {
            ...
            if (latOFileMask) {
                if (!MaskMatch (latOFileMask, lfn, fn))
                    HError(2319,"HLRescore: LATOFILEMASK %s has no match with segemnt %s", latOFileMask, fn);
            } else {
                strcpy (lfn, fn);
            }
            ...
            if (!file) 
                HError (9999, "DoRecognition: Could not open file %s for lattice output", latfn);
            if (!latOutForm)
