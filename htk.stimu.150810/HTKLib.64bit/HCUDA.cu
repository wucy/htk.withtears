...
__global__ void HKern_UpdateActParmStandardLHUCCUDA(NFloat *srcVec, int len, NFloat *dstVec) {
    int nowIdx = threadIdx.x;
    int thdNum = blockDim.x;
    float old_a;
    while (nowIdx < len) {
        old_a = - log(2 / dstVec[nowIdx] - 1);

        nowIdx += thdNum;
    }
}
void UpdateActParmStandardLHUCCUDA(NFloat *srcVec, int len, NFloat *dstVec) {
    HKern_UpdateActParmStandardLHUCCUDA<<<1,THREADPERBLOCK>>>(srcVec, len, dstVec); 
}

__global__ void HKern_ApplyTrPSigmoidActStandardLHUCCUDA(NFloat *errPtr, NFloat *srcPtr, NFloat *etaPtr, int row, int col, Boolean accFlag, NFloat *dEtaPtr) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx, colIdx, thdNum, base, idx, incr, pos, step;
    float tmpVal = 0;
    ...
    if (thdIdx < thdNum && colIdx < col) {
        ...
        while (idx < row) {
            /* tmpPtr[thdIdx] += errPtr[pos] * srcPtr[pos] / etaPtr[colIdx]; */
            if (etaPtr[colIdx] != 0.0) {
                tmpVal = errPtr[pos] * 1.0 / etaPtr[colIdx] * srcPtr[pos];
                tmpPtr[thdIdx] += tmpVal * (1 - etaPtr[colIdx]) * etaPtr[colIdx];
            }
            pos += step;
            idx += thdNum;
        }
...
void ApplyTrPSigmoidActStandardLHUCCUDA(NFloat *errPtr, NFloat *srcPtr, NFloat *etaPtr, int row, int col, Boolean accFlag, NFloat *dEtaPtr) {
    int nBlocks, sBytes;

    nBlocks = col;
    sBytes = sizeof(NFloat) * THREADPERBLOCK;
    if (nBlocks > MAXBLOCKNUM)
        HError(9999, "ApplyTrPSigmoidActStandardLHUCCUDA: Block number exceeds the maximum");
    HKern_ApplyTrPSigmoidActStandardLHUCCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(errPtr, srcPtr, etaPtr, row, col, accFlag, dEtaPtr);
}
...
__global__ void HKern_ApplyActLHUCPenaltyLocalSoftActiDeDyCUDA(NFloat * y, NFloat  * dest_dy, NFloat * d_lhuc_surface, NFloat * lhuc, float lhuc_penalty, int nodeNum, int batLen, NFloat * weight_norms) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx = threadIdx.x; 
    int blkIdx = blockIdx.x;
    int thdNum = min(blockDim.x, nodeNum);
    
    int idx, base, blk_node_id, blk_samp_id, blk_surface_offset, incr, pos;
    
    float blk_lhuced_acti, blk_raw_acti;
    float regVal = 0;
    float tmpVal = 0;
    float diff = 0;
    if (thdIdx < thdNum && blkIdx < batLen * nodeNum) {
        blk_node_id = blkIdx % nodeNum;
        blk_lhuced_acti = y[blkIdx] * weight_norms[blk_node_id];

        blk_surface_offset = blk_node_id * nodeNum;
        blk_samp_id = blkIdx / nodeNum;
        base = blk_samp_id * nodeNum;
        idx = thdIdx;
        while (idx < nodeNum) {
            diff = (blk_lhuced_acti - y[base + idx] * weight_norms[idx]) / 20;
            tmpVal += diff * d_lhuc_surface[blk_surface_offset + idx];
            //regVal += diff * diff * d_lhuc_surface[blk_surface_offset + idx];
            idx += thdNum;
        }
        ...
        if (thdIdx == 0) {
            //blk_raw_acti = lhuc[blk_node_id];
            //sep_d_lhuc[blkIdx] = tmpPtr[0] * lhuc[blk_node_id];
            dest_dy[blkIdx] += lhuc_penalty * tmpPtr[0] * lhuc[blk_node_id] * weight_norms[blk_node_id] / 20;
        }
    }
}
void ApplyActLHUCPenaltyLocalSoftActiDeDyCUDA(NFloat * lhuc, NFloat * dest_dy, NFloat * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NFloat * y, NFloat * weight_norms) {
    int nBlocks = batLen * nodeNum;
    int sBytes = THREADPERBLOCK * sizeof(NFloat);
    if (nBlocks > MAXBLOCKNUM)
        HError(9999, "ApplyActLHUCPenaltyLocalSoftActiDeDyCUDA: Block number exceeds the maximum");
    HKern_ApplyActLHUCPenaltyLocalSoftActiDeDyCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(y, dest_dy, d_lhuc_surface, lhuc, lhuc_penalty, nodeNum, batLen, weight_norms);
}

__global__ void HKern_ApplyActLHUCPenaltyLocalSoftActiStandardLHUCCUDA(NFloat * sep_d_lhuc, NFloat * y, NFloat * d_lhuc_surface, NFloat * lhuc, int nodeNum, int batLen, NFloat * penalty_val, NFloat * weight_norms) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx = threadIdx.x; 
    int blkIdx = blockIdx.x;
    int thdNum = min(blockDim.x, nodeNum);
    ...
    if (thdIdx < thdNum && blkIdx < batLen * nodeNum) {
        ...
        tmpPtr[thdIdx] = tmpVal;
        //atomicAdd(penalty_val, regVal * 4);
        __syncthreads();
        for (idx = thdNum; idx > 1; idx = incr) {
            incr = idx / 2;
            if (idx % 2 != 0) {
        ...
        if (thdIdx == 0) {
            penalty_val[blkIdx] = 0;
            blk_raw_acti = y[blkIdx] / lhuc[blk_node_id];
            sep_d_lhuc[blkIdx] = tmpPtr[0] * blk_raw_acti * weight_norms[idx] / 20 * (1 - lhuc[blk_node_id]) * lhuc[blk_node_id];
        }
        __syncthreads();
        //atomicAdd(penalty_val + blkIdx, regVal * 4);
    }
}


__global__ void HKern_ApplyActLHUCPenaltyLocalSoftActiCUDA(NFloat * sep_d_lhuc, NFloat * y, NFloat * d_lhuc_surface, NFloat * lhuc, int nodeNum, int batLen, NFloat * penalty_val, NFloat * weight_norms) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx = threadIdx.x; 
    int blkIdx = blockIdx.x;
    int thdNum = min(blockDim.x, nodeNum);
    ...
    if (thdIdx < thdNum && blkIdx < batLen * nodeNum) {
        ...
        if (thdIdx == 0) {
            penalty_val[blkIdx] = 0;
            blk_raw_acti = y[blkIdx] / lhuc[blk_node_id];
            sep_d_lhuc[blkIdx] = tmpPtr[0] * blk_raw_acti * weight_norms[idx] / 20;
        }
        __syncthreads();
        //atomicAdd(penalty_val + blkIdx, regVal * 4);
    }
...
__global__ void HKern_ApplyActLHUCPenaltyLocalSoftActiMergeCUDA(NFloat * d_lhuc, NFloat * dest_d_lhuc, float lhuc_penalty, int nodeNum, int batLen) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx = threadIdx.x;
    int blkIdx = blockIdx.x;
    int thdNum = min(blockDim.x, batLen);
...
__global__ void HKern_ApplyActLHUCPenaltyLocalSoftActiMergePenaltyCUDA(NFloat * dest_penalty, NFloat * penalty_vals, int nodeNum, int batLen) {
    extern __shared__ NFloat tmpPtr[];
    int thdIdx = threadIdx.x;
    int blkIdx = blockIdx.x;
    int thdNum = min(blockDim.x, batLen * nodeNum);
    int idx, pos, my_node_id, incr, step, tot;
    if (thdIdx < thdNum) {
        idx = blkIdx + thdIdx;
        step = thdNum;
        ...
        tot = nodeNum * batLen;
        while (idx < tot) {
            tmpPtr[thdIdx] += penalty_vals[idx];
            idx += step;
        }
        __syncthreads();
...
void ApplyActLHUCPenaltyLocalSoftActiCUDA(NFloat * lhuc, NFloat * dest_d_lhuc, NFloat * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NFloat * penalty_val, NFloat * sep_penalty_val, NFloat * sep_d_lhuc, NFloat * y, NFloat * weight_norms) {
    int nBlocks = batLen * nodeNum;
    int sBytes = THREADPERBLOCK * sizeof(NFloat);
    if (nBlocks > MAXBLOCKNUM)
        HError(9999, "ApplyActLHUCPenaltyLocalSoftActiCUDA: Block number exceeds the maximum");
    HKern_ApplyActLHUCPenaltyLocalSoftActiCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(sep_d_lhuc, y, d_lhuc_surface, lhuc, nodeNum, batLen, sep_penalty_val, weight_norms);
    nBlocks = nodeNum;
    HKern_ApplyActLHUCPenaltyLocalSoftActiMergeCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(sep_d_lhuc, dest_d_lhuc, lhuc_penalty, nodeNum, batLen);
    HKern_ApplyActLHUCPenaltyLocalSoftActiMergePenaltyCUDA<<<1, THREADPERBLOCK, sBytes>>>(penalty_val, sep_penalty_val, nodeNum, batLen);
}

void ApplyActLHUCPenaltyLocalSoftActiStandardLHUCCUDA(NFloat * lhuc, NFloat * dest_d_lhuc, NFloat * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NFloat * penalty_val, NFloat * sep_penalty_val, NFloat * sep_d_lhuc, NFloat * y, NFloat * weight_norms) {
    int nBlocks = batLen * nodeNum;
    int sBytes = THREADPERBLOCK * sizeof(NFloat);
    if (nBlocks > MAXBLOCKNUM)
        HError(9999, "ApplyActLHUCPenaltyLocalSoftActiStandardLHUCCUDA: Block number exceeds the maximum");
    HKern_ApplyActLHUCPenaltyLocalSoftActiStandardLHUCCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(sep_d_lhuc, y, d_lhuc_surface, lhuc, nodeNum, batLen, sep_penalty_val, weight_norms);
    nBlocks = nodeNum;
    HKern_ApplyActLHUCPenaltyLocalSoftActiMergeCUDA<<<nBlocks, THREADPERBLOCK, sBytes>>>(sep_d_lhuc, dest_d_lhuc, lhuc_penalty, nodeNum, batLen);
    HKern_ApplyActLHUCPenaltyLocalSoftActiMergePenaltyCUDA<<<1, THREADPERBLOCK, sBytes>>>(penalty_val, sep_penalty_val, nodeNum, batLen);
}
