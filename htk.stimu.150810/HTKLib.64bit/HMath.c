...
void UpdateActParmStandardLHUC(NVector *srcVec, int len, NVector *dstVec) {
#ifdef CUDA
    UpdateActParmStandardLHUCCUDA(srcVec->devElems, len, dstVec->devElems);
#else
    HError(9999, "Error: UpdateActParmStandardLHUC");
#endif
}

void ApplyTrPSigmoidActStandardLHUC(NMatrix *errMat, NMatrix *srcMat, NVector *etaVec, int row, int col, Boolean accFlag, NVector *dEtaVec) {
    /* safety check */
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && srcMat->colNum == col && etaVec->vecLen == col && dEtaVec->vecLen == col))
            HError(9999, "ApplyTrPSigmoidAct: Input column number incompatible");
...
#ifdef CUDA
    ApplyTrPSigmoidActStandardLHUCCUDA(errMat->devElems, srcMat->devElems, etaVec->devElems, row, col, accFlag, dEtaVec->devElems);
#else
    HError(9999, "ApplyTrPSigmoidActStandardLHUC: No implementation.");
#endif
}



void ApplyActLHUCPenaltyLocalSoftActiDeDy(NVector * lhuc, NMatrix * dest_dy, NMatrix * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NMatrix * y, NVector * weight_norms) {
#ifdef CUDA
    ApplyActLHUCPenaltyLocalSoftActiDeDyCUDA(lhuc->devElems, dest_dy->devElems, d_lhuc_surface->devElems, nodeNum, batLen, lhuc_penalty, y->devElems, weight_norms->devElems);
#else
    HError(9999, "ApplyActLHUCPenaltyLocalSoftActiDeDy: not support CPU.");
#endif
}
...
void ApplyActLHUCPenaltyLocalSoftActiStandardLHUC(NVector * lhuc, NVector * dest_d_lhuc, NMatrix * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NVector * penalty_val, NVector * sep_penalty_val, NMatrix * sep_d_lhuc, NMatrix * y, NVector * weight_norms) {
#ifdef CUDA
    ...
    NFloat * sep_penalty_val_elems = sep_penalty_val->devElems;
    NFloat * sep_d_lhuc_elems = sep_d_lhuc->devElems;
    NFloat * y_elems = y->devElems;
    NFloat * weight_norms_elems = weight_norms->devElems;
    //printf("inmath=%f\n", lhuc_penalty);
    ApplyActLHUCPenaltyLocalSoftActiStandardLHUCCUDA(lhuc_elems, dest_d_lhuc_elems, d_lhuc_surface_elems, nodeNum, batLen, lhuc_penalty, penalty_val_elems, sep_penalty_val_elems, sep_d_lhuc_elems, y_elems, weight_norms_elems);
#else
...
void ApplyActLHUCPenaltyLocalSoftActi(NVector * lhuc, NVector * dest_d_lhuc, NMatrix * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NVector * penalty_val, NVector * sep_penalty_val, NMatrix * sep_d_lhuc, NMatrix * y, NVector * weight_norms) {
#ifdef CUDA
    NFloat * lhuc_elems = lhuc->devElems;
    NFloat * dest_d_lhuc_elems = dest_d_lhuc->devElems;
    NFloat * d_lhuc_surface_elems = d_lhuc_surface->devElems;
    ...
    NFloat * weight_norms_elems = weight_norms->devElems;
    //printf("inmath=%f\n", lhuc_penalty);
    ApplyActLHUCPenaltyLocalSoftActiCUDA(lhuc_elems, dest_d_lhuc_elems, d_lhuc_surface_elems, nodeNum, batLen, lhuc_penalty, penalty_val_elems, sep_penalty_val_elems, sep_d_lhuc_elems, y_elems, weight_norms_elems);
#else
    HError(9999, "ApplyActLHUCPenaltyLocalSoft: not support CPU.");
#endif
}
