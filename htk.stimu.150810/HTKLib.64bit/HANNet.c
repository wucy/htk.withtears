...
void ForwardPropBatch(ANNSet *annSet, int batLen, int *CMDVecPL) {
    ...
    while (curAI != NULL) {
        ...
        if (!STP()->acti_surface) {
            int dim = annDef->layerList[0]->nodeNum;
        ...
        if (STP()->weight_norms == NULL) {
            STP()->weight_norms = CreateNVector(STP()->heap, annDef->layerList[1]->inputDim);
        }
        ...
        if (STP()->lhuc_regVal == NULL) {
            STP()->lhuc_sep_regVal = CreateNVector(STP()->heap, annDef->layerList[0]->nodeNum * GetNBatchSamples());
...
exit(1);*/
                ...
                if (i < annDef->layerNum - 1) {
                    ...
                    CalcSumActi(STP()->sum_xFeaMat, layerElem->yFeaMats[j], STP()->weight_norms, batLen, layerElem->nodeNum);
                    /*if (STP()->newepoch == 1) {
                    
                    STP()->newepoch = 0;
                    
                    SyncNVectorDev2Host(STP()->sum_xFeaMat);
                    SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                    SyncNVectorDev2Host(STP()->weight_norms);
                    int ff, sid=10;
                    float xx = 0;
                    for (ff = 0; ff < 1024; ++ ff) {
                        printf("%f ", STP()->weight_norms->vecElems[ff]);
                    }
                    printf("\n");
                    
                    for (ff = 0; ff < 1024; ++ ff) {
                        if (layerElem->yFeaMats[j]->matElems[ff + sid * 1024] < 0.0000001) {
                            layerElem->yFeaMats[j]->matElems[ff + sid * 1024] = 0.0000001;
                        }
                        //printf("%f ", layerElem->yFeaMats[j]->matElems[ff + sid * 1024] * STP()->weight_norms->vecElems[ff] / STP()->sum_xFeaMat->vecElems[sid]);
                        xx += layerElem->yFeaMats[j]->matElems[ff + sid * 1024] * STP()->weight_norms->vecElems[ff];
                    }
                    printf("\n");
                    for (ff = 0; ff < 1024; ++ ff) {
                        //printf("%f ", STP()->acti_surface->matElems[(int)(STP()->phoneidx_vec->vecElems[sid]) + ff * 46]);
                    }
                    float kl = 0;
                    for (ff = 0; ff < 1024; ++ ff) {
                        float g, f;
                        g = STP()->acti_surface->matElems[(int)(STP()->phoneidx_vec->vecElems[sid]) + ff * 46];
                        f = layerElem->yFeaMats[j]->matElems[ff + sid * 1024] * STP()->weight_norms->vecElems[ff] / STP()->sum_xFeaMat->vecElems[sid];
                        kl += g * log(g/f);
                    }
                    printf("\n");
                    printf("sumcuda=%f sum_cpu=%f klcpu=%f phname=%s\n", STP()->sum_xFeaMat->vecElems[sid], xx, kl, STP()->phone_names[(int)(STP()->phoneidx_vec->vecElems[sid])]);
                    
                    }*/
                    
                    CalcStimuKL(STP()->klvec, layerElem->yFeaMats[j], 
                            STP()->weight_norms, STP()->sum_xFeaMat, STP()->acti_surface, STP()->phoneidx_vec, 
                            batLen, layerElem->nodeNum, STP()->num_phone);
                    SyncNVectorDev2Host(STP()->klvec);
                    STP()->accumKL += STP()->klvec->vecElems[0];
                    
                    //STP()->klvec->vecElems[0] = 10;
                    //printf("batLen=%d, kl=%f\n", batLen, STP()->klvec->vecElems[0]);
                    //exit(0);
                }
                ...
                if (strcmp(STP()->visual_fn, "") && i < annDef->layerNum - 1) {
                    ...
                    for (kk = 0; kk < batLen; ++ kk) {
                        ...
                        for (jj = 0; jj < layerElem->nodeNum; ++ jj) {
                            fprintf(tmp_f, " %f", layerElem->yFeaMats[j]->matElems[kk * layerElem->nodeNum + jj]); // * STP()->weight_norms->vecElems[jj]);
                        }
                        fprintf(tmp_f, "\n");
                    }
                    fclose(tmp_f);
            ...
            for (j = 1; j <= n; ++j) {
                ...
                if (layerElem->roleKind == OUTRK) {
                    ...
                    if (STP()->stimu_dxFeaMat == NULL) {
                        STP()->stimu_dxFeaMat = CreateNMatrix(STP()->heap, GetNBatchSamples(), layerElem->nodeNum);
                    }
                    ...
                    printf("\n");
                    */
                    /*printf("%f %f\n", STP()->phonepos->matElems[0], STP()->phonepos->matElems[1]);
                    for (kk = 0; kk < 32; kk ++)
                    {
                        for (mm = 0; mm < 32; mm ++)
                        {
                            printf("%f ", STP()->acti_surface->matElems[(kk * 32 + mm) * 46 + 2]);
                        }
                        printf("\n");
                    }
                    exit(0);
                    */
                    ...
                    /*
                    SyncNVectorDev2Host(STP()->weight_norms);
                    SyncNMatrixDev2Host(annDef->layerList[i + 1]->wghtMat);
                    float sum5 = 0, sum201 = 0;
                    int ff;
                    for (ff = 0; ff < annDef->layerList[i + 1]->nodeNum; ++ ff) {
                        printf("%f ", annDef->layerList[i + 1]->wghtMat->matElems[ff * annDef->layerList[i + 1]->inputDim + 5]);
                        sum5 += annDef->layerList[i + 1]->wghtMat->matElems[ff * annDef->layerList[i + 1]->inputDim + 5] *
                                annDef->layerList[i + 1]->wghtMat->matElems[ff * annDef->layerList[i + 1]->inputDim + 5];
                        sum201 += annDef->layerList[i + 1]->wghtMat->matElems[ff * annDef->layerList[i + 1]->inputDim + 201] *
                                annDef->layerList[i + 1]->wghtMat->matElems[ff * annDef->layerList[i + 1]->inputDim + 201];
                    }
                    printf("\n");
                    printf("out_dim=%d in_dim=%d 5cpu=%f 201cpu=%f 5gpu=%f 201gpu=%f\n", 
                            annDef->layerList[i + 1]->nodeNum, annDef->layerList[i + 1]->inputDim, sqrt(sum5), sqrt(sum201),
                            STP()->weight_norms->vecElems[5], STP()->weight_norms->vecElems[201]);
                    exit(0);
                    */
                    StimuGrad(layerElem->yFeaMats[j], 
                            batLen, layerElem->nodeNum, STP()->acti_surface, STP()->num_phone, STP()->phoneidx_vec, STP()->stimu_dxFeaMat, STP()->weight_norms);
                    
                    /*
                    int kk;
                    SyncNMatrixDev2Host(STP()->stimu_dxFeaMat);
                    SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                    SyncNMatrixDev2Host(dyFeaMat);
                    //for (kk = 0 + 80 * 1000; kk < 90 * 1000; kk += 1000)
                    for (kk = 0; kk < 10; kk += 1)
                    {
                        printf("stimud=%f, orid=%f,ori=%f,ph=%d\t", STP()->stimu_dxFeaMat->matElems[kk], dyFeaMat->matElems[kk], layerElem->yFeaMats[j]->matElems[kk], (int)(STP()->phoneidx_vec->vecElems[0]));
                    }
                    
                    printf("\npenal=%f\n", STP()->stimu_penalty);
                    //exit(0);
                    */
                    //printf("dymatcol=%d,dymatrow=%d\n", dyFeaMat->colNum, dyFeaMat->rowNum);
                    AddScaledNMatrix(STP()->stimu_dxFeaMat, batLen, layerElem->nodeNum, STP()->stimu_penalty, dyFeaMat);
                    }
                    //cw564 - stimu -- end
...
exit(1);*/
                    ...
                    case PSIGMOIDAF:
                        if (!STP()->standardLHUC) {
                            ...
                            ApplyTrPSigmoidActStandardLHUC(dyFeaMat, layerElem->yFeaMats[j], layerElem->actFunInfo.actParmVec[1], batLen, layerElem->nodeNum, accFlag, layerElem->trainInfo->gradInfo->actParmVec[1]);
                        }
                        break;
                    case PARMSIGMOIDAF:
                    ...
                    if (1) {
                        ...
                        //HError(9999, "");
                        //if ()
                        if (STP()->lhuc_penalty_type == 1 && STP()->lhuc_penalty != 0.0) {
                            if (STP()->d_lhuc == NULL) {
                            ...
                            if (!STP()->standardLHUC) {
                                ApplyActLHUCPenaltyLocalSoftActi(layerElem->actFunInfo.actParmVec[1], layerElem->trainInfo->gradInfo->actParmVec[1], 
                                    STP()->lhuc_surface, layerElem->nodeNum, batLen, STP()->lhuc_penalty, STP()->lhuc_regVal, STP()->lhuc_sep_regVal, 
                                    STP()->d_lhuc, layerElem->yFeaMats[j], STP()->weight_norms);
                            }
                            else {
                                ApplyActLHUCPenaltyLocalSoftActiStandardLHUC(layerElem->actFunInfo.actParmVec[1], layerElem->trainInfo->gradInfo->actParmVec[1],
                                    STP()->lhuc_surface, layerElem->nodeNum, batLen, STP()->lhuc_penalty, STP()->lhuc_regVal, STP()->lhuc_sep_regVal,
                                    STP()->d_lhuc, layerElem->yFeaMats[j], STP()->weight_norms);
                            }
                            ApplyActLHUCPenaltyLocalSoftActiDeDy(layerElem->actFunInfo.actParmVec[1], dyFeaMat, STP()->lhuc_surface, 
                                layerElem->nodeNum, batLen, STP()->lhuc_penalty, layerElem->yFeaMats[j], STP()->weight_norms);
                            //ShowNVector(layerElem->trainInfo->gradInfo->actParmVec[1]);
...
void InitStimu(MemHeap * heap) {
    ...
    */
    ...
    if (nSTParm > 0) {
        if (GetConfStr(cSTParm, nSTParm, "POSFN", buf)) {
        ...
        if (GetConfInt(cSTParm, nSTParm, "STANDARDLHUC", &intVal)) {
            stp.standardLHUC = intVal;
        }
...
int LabName2STPIdx(char * labname) {
    ...
    for (i = 0; i < stp.num_phone; ++ i) {
        ...
        //    return i;
        //}
        if (strlen(stp.phone_names[i]) > strlen(mono_name)) {
            continue;
        }
        if (!strncmp(stp.phone_names[i], mono_name, strlen(stp.phone_names[i]))) {
            return i;
        }
    }
