...
void InitANNSet(HMMSet *hset) {
    ...
    /* cz277 - xform */
    hset->annSet->nRplNMat = 0;
    for (i = 1; i < MAXNUMRPLNMAT) {
        hset->annSet->rplNMats[i] = NULL;
    }
    hset->annSet->nRplNVec = 0;
    for (i = 1; i < MAXNUMRPLNVEC) {
        hset->annSet->rplNVecs[i] = NULL;
    }
}
...
typedef enum {   /* Only a character big !! */
   ...
   OPERAND, ACTIVATION, INPUTFEA, MATRIX, ROWSHARE, UPDATE, LEARNRATE, SQUAREGRAD, VECTOR,              /* cz277 - ANN: terms for ANN layer definition */
   RPLIDX,	/* cz277 - xform */
...
static struct {
   ...
   { { "BEGINHMM", BEGINHMM }, { "USE", USEMAC }, 
     ...
     {"OPERAND", OPERAND}, {"ACTIVATION", ACTIVATION}, {"INPUTFEA", INPUTFEA}, {"MATRIX", MATRIX}, {"ROWSHARE", ROWSHARE}, {"UPDATE", UPDATE}, {"LEARNRATE", LEARNRATE}, {"SQUAREGRAD", SQUAREGRAD}, {"VECTOR", VECTOR}, 
     {"RPLIDX", RPLIDX}, 	/* cz277 - xform */
...
static NMatrix *GetNMatrix(HMMSet *hset, Source *src, Token *tok)
{
    ...
    if (tok->sym == RPLIDX) {
        ...
        CheckRplNMatIdx(wghtMat->rplIdx);
        if (hset->annSet->rplNMats[wghtMat->rplIdx] != NULL) {
            HMError(src, "Replaceable part %d already set", wghtMat->rplIdx);
            return NULL;
        }
        hset->annSet->rplNMats[wghtMat->rplIdx] = wghtMat;
        if (wghtMat->rplIdx > hset->annSet->nRplNMat) {
            hset->annSet->nRplNMat = wghtMat->rplIdx;
        }
        /* get the next token */
        if (GetToken(src, tok) < SUCCESS) {
            HMError(src, "GetToken failed");
...
static NVector *GetNVector(HMMSet *hset, Source *src, Token *tok)
{
    ...
    if (tok->sym == RPLIDX) {
        ...
        CheckRplNVecIdx(biasVec->rplIdx);
        if (hset->annSet->rplNVecs[biasVec->rplIdx] != NULL) {
            HMError(src, "Replaceable part %d already set", biasVec->rplIdx);
            return NULL;
        }
        hset->annSet->rplNVecs[biasVec->rplIdx] = biasVec;
        if (biasVec->rplIdx > hset->annSet->nRplNVec) {
            hset->annSet->nRplNVec = biasVec->rplIdx;
        }
        /* get the next token */
        if (GetToken(src, tok) < SUCCESS) {
            HMError(src, "GetToken failed");
...
/* cz277 - xform */
static void GetActFunParams(Source *src, Token *tok, LELink layerElem) {
    ...
    if (tok->sym == MACRO && tok->macroType == 'k') {
        ...
        */
        ...
        if (tok->sym == ACTIVATION) {
            ...
            GetActFunParams(src, tok, layerElem);	/* cz277 - xform */
            /* read next keyword */
            if (GetToken(src, tok) < SUCCESS) {
                HMError(src, "GetToken failed");
                return NULL;
        ...
        if (tok->sym == INPUTFEA) {
            ...
            HMError(src, "<INPUTFEA> symbol expected");
        }
        if (tok->sym == ROWSHARE) {
        ...
        /* <MATRIX> nrows ncols */
        if (tok->sym != MATRIX) {
            HMError(src, "<MATRIX> symbol expected");
            return NULL;
        }
...
/* cz277 - max norm */
        ...
        /* <VECTOR> size(nrows) */
        if (tok->sym != VECTOR) {
            HMError(src, "<VECTOR> symbol expected");
            return NULL;
        }
...
/*  */
ReturnStatus SetupTargetMapList(HMMSet *hset, char *mapFN) {
    Source src;
