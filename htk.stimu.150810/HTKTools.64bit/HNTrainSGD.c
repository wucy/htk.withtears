    ...
    while (curAI != NULL) {
        ...
        for (i = annDef->layerNum - 1; i >= 0; --i) {
            ...
            if (learnRate != 0.0) {
                ...
                if (layerElem->trainInfo->updtFlag & WEIGHTUK)
                    ScaleNMatrix(learnRate / scale, layerElem->inputDim, layerElem->nodeNum, layerElem->trainInfo->gradInfo->wghtMat);
                if (layerElem->trainInfo->updtFlag & BIASUK)
                    ScaleNVector(learnRate / scale, layerElem->nodeNum, layerElem->trainInfo->gradInfo->biasVec);
            }
...
printf("\n\n");*/
			...
			if (actFunParmUpdtCode & (int) pow(2, j - 1))
                            if (!STP()->standardLHUC) {
            //printf("hahaha\n");                    ShowNVector(layerElem->trainInfo->updtInfo->actParmVec[j]);exit(0);
                                ...
                                UpdateActParmStandardLHUC(
                                    layerElem->trainInfo->updtInfo->actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->actFunInfo.actParmVec[j]);
                            }
                    }
                }
            }
