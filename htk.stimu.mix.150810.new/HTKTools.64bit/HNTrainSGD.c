...
/* cz277 - varlen */
static Boolean normWeightDecay = FALSE;
static Boolean normMomentum = FALSE;
static Boolean normGradientClip = FALSE;
...
static float actfGradScale = 1.0;		/* cz27 - pact */

/* cz277 - gradlim */
static float actfUpdtPosLim = 0.32;		/* cz277 - pact */
static float actfUpdtNegLim = -0.32;             /* cz277 - pact */
...
/* chaopig - lafhacky */
static int actFunParmUpdtCode = 65535;
enum _ActFunParmUpdtKind {ACTFUNPARM1UK = 1, ACTFUNPARM2UK = 2, ACTFUNPARM3UK = 4};
typedef enum _ActFunParmUpdtKind ActFunParmUpdtKind;
...
static Boolean optActfUpdtLim = TRUE;		/* cz277 - pact */
/* cz277 - gradlim */
static Boolean optWghtUpdtLim = FALSE;
static Boolean optBiasUpdtLim = FALSE;
/* cz277 - max norm */
...
void SetConfParms(void)
{
    ...
    if (nParm > 0) {
        ...
        /* cz277 - pact */
        if (GetConfFlt(cParm, nParm, "ACTFUNUPDATELIM", &doubleVal)) {
            if (doubleVal <= 0.0) {
                optActfUpdtLim = FALSE;
            }
            else {
                optActfUpdtLim = TRUE;
                actfUpdtPosLim = (float) doubleVal;
                actfUpdtNegLim = -1.0 * actfUpdtPosLim;
            }
        }
        /* cz277 - gradlim */
        ...
        /* chaopig - lafhacky */
        if (GetConfInt(cParm, nParm, "ACTFUNPARMUPDATECODE", &intVal)) {
            actFunParmUpdtCode = intVal;
        }
        if (GetConfFlt(cParm, nParm, "ACTFUNGRADSCALE", &doubleVal)) {
            actfGradScale = doubleVal;
        } 
...
inline int is_sil(int lab) {
    if (lab == 777 || lab == 778 || lab == 779)
        return 1;
    return 0;
}
...
void PrintCriteria(CriteriaInfo *criteria, char *setid) {
    ...
    PrintStimuKL((int) (criteria->tSampAcc)); //cw564 - stimu
    if (optTrainMode == FRAMETM) {
        ...
        if (showObjFunKind & XENTOF) {
            printf("\t\t%s Cross Entropy/Frame = %.5f\n", setid, criteria->XENTAcc / criteria->tSampAcc);
            PrintLHUCRegVal((int) (criteria->tSampAcc), criteria->XENTAcc, STP()->lhuc_penalty); //cw564 - stimu
            PrintSilPerformance(); //cw564 - stimu -- mix
        }
        if (showObjFunKind & MMSEOF) {
...
void UpdateANNParamSGD(ANNSet *annSet, float learnRate, float momentum, float weightDecay, int uttLen) {
    int i, j;
    AILink curAI;
    ADLink annDef;
    LELink layerElem;
    /* cz277 - max norm */
    NFloat extWghtAlpha;
    float scale, posActfunClip, negActfunClip;

    /* cz277 - varlen */
    if (normWeightDecay) 
        weightDecay = weightDecay / GetNBatchSamples() * uttLen;    
    ...
    while (curAI != NULL) {
        ...
        for (i = annDef->layerNum - 1; i >= 0; --i) {
            ...
            layerElem = annDef->layerList[i];
            scale = layerElem->trainInfo->tDrvCnt;
                ...
                if (layerElem->trainInfo->updtFlag & ACTFUNUK) {
                    for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
                        if (layerElem->actFunInfo.actParmVec[j] != NULL) {
                            AddScaledNVector(layerElem->actFunInfo.actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, weightDecay, layerElem->trainInfo->gradInfo->actParmVec[j]);
                        }
                    }
                }
            ...
            if (learnRate != 0.0) {
                ...
                if (layerElem->trainInfo->updtFlag & ACTFUNUK) {
                    for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
                        if (layerElem->actFunInfo.actParmVec[j] != NULL) {
                            //ScaleNVector(1.0 / GetNBatchSamples(), layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->trainInfo->gradInfo->actParmVec[j]);
                            ScaleNVector(actfGradScale * learnRate / scale, layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->trainInfo->gradInfo->actParmVec[j]);
                            //printf("%f %f %f\n", actfGradScale, learnRate, scale);
                        }
                    }
                }
                //cw564
                ...
                if (i < annDef->layerNum - 1) {
                    x = 1.0 / STP()->resdnn_var;
                }
                if (layerElem->trainInfo->updtFlag & WEIGHTUK)
                    ScaleNMatrix(learnRate / scale * x, layerElem->inputDim, layerElem->nodeNum, layerElem->trainInfo->gradInfo->wghtMat);
                if (layerElem->trainInfo->updtFlag & BIASUK)
                    ScaleNVector(learnRate / scale * x, layerElem->nodeNum, layerElem->trainInfo->gradInfo->biasVec);
            }
                ...
                if (layerElem->trainInfo->updtFlag & ACTFUNUK) {
                    for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
                        if (layerElem->actFunInfo.actParmVec[j] != NULL) {
                            if (scale != 1.0)
                                ScaleNVector(actfGradScale * 1.0 / scale, layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->trainInfo->gradInfo->actParmVec[j]);
                            MulNVector(layerElem->trainInfo->gradInfo->actParmVec[j], layerElem->trainInfo->nlrInfo->actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->trainInfo->gradInfo->actParmVec[j]);
                        }
                    }
                }
                ...
                if (layerElem->trainInfo->updtFlag & WEIGHTUK) {
                    if (scale != 1.0)
                        ScaleNMatrix(1.0 / scale, layerElem->inputDim, layerElem->nodeNum, layerElem->trainInfo->gradInfo->wghtMat);
                ...
                if (layerElem->trainInfo->updtFlag & BIASUK) {
                    if (scale != 1.0)
                        ScaleNVector(1.0 / scale, layerElem->nodeNum, layerElem->trainInfo->gradInfo->biasVec);
            ...
            /* cz277 - pact */
            if (optActfUpdtLim && (layerElem->trainInfo->updtFlag & ACTFUNUK)) {
                /* cz277 - varlen */
                posActfunClip = actfUpdtPosLim / GetCurClipScalingFactor(i);
                negActfunClip = actfUpdtNegLim / GetCurClipScalingFactor(i);
                if (normGradientClip) {
                    posActfunClip = posActfunClip / GetNBatchSamples() * uttLen;
                    negActfunClip = negActfunClip / GetNBatchSamples() * uttLen;
                }
                for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
                    if (layerElem->actFunInfo.actParmVec[j] != NULL) {
                        ClipNVectorVals(layerElem->trainInfo->gradInfo->actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, actfUpdtPosLim / GetCurClipScalingFactor(i), actfUpdtNegLim / GetCurClipScalingFactor(i), layerElem->trainInfo->gradInfo->actParmVec[j]);
                    }
                }
            }
            ...
            if (layerElem->trainInfo->updtFlag & ACTFUNUK) {
                for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
                    if (layerElem->actFunInfo.actParmVec[j] != NULL) {
                        ScaledSelfAddNVector(layerElem->trainInfo->gradInfo->actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, momentum, layerElem->trainInfo->updtInfo->actParmVec[j]);
                    }
                }
            }
...
/*erray9*/
/*printf("VECTOR j = %d\n", j);
ShowNVector(layerElem->trainInfo->updtInfo->actParmVec[j]);
printf("\n\n");*/
                        /* chaopig - lafhacky */
                        /*if (actFunParmUpdtCode & ACTFUNPARM1UK)*/	/* chaopig - lafhacky */
                        if (actFunParmUpdtCode & (int) pow(2, j - 1)) {
                            AddNVector(layerElem->trainInfo->updtInfo->actParmVec[j], layerElem->actFunInfo.actParmVec[j]->vecLen, layerElem->actFunInfo.actParmVec[j]);
                            //printf("%d\n", STP()->mixnodes);
                            //MixValidate(layerElem->actFunInfo.actParmVec[j], STP()->mixnodes, layerElem->actFunInfo.actParmVec[j]->vecLen);
                            //SyncNVectorDev2Host(layerElem->actFunInfo.actParmVec[j]);
                            //int kk = 0;
                            //for (kk = 0; kk < STP()->mixnodes; ++ kk) {
                            //    printf("%f %f\n", layerElem->actFunInfo.actParmVec[j]->vecElems[kk * 6], layerElem->actFunInfo.actParmVec[j]->vecElems[kk * 6 + 1]);
                            //}
                        }
                    }
                }
...
/*erray9*/
/*if (layerElem->actFunInfo.actParmVec[1] != NULL) {
exit(1);
}*/
...
void BatchLevelTrProcess(int curEpochNum) {
    ...
    while (!finish) {
		...
		/* cz277 - semi */
                ...
                stClock = clock();
                UpdateANNParamSGD(hset.annSet, retNegLR, momentum, weightDecay, nLoaded);
                edClock = clock();
                fbPropClock += edClock - stClock;
                /* update updtCnt */
                ++updtCnt;
...
void UtterLevelTrProcess(int curEpochNum) {
    ...
    while (!finish) {
        ...
        while ((!finish) && (updtUttCnt < numPerUpdt)) {
            ...
            InstallRplParts(&hset, uttElem);	/* cz277 - xform */
            ...
            if (optTrainMode == SEQTM) {
                ...
                /*InstallRplParts(&hset, uttElem, -1);*/    /* cz277 - xform */
                fbInfo.uFlags = cacheTr[1]->labelInfo->uFlags;
            }
            while ((!finish) && uttCnt > 0) {
                ...
                for (i = 1; i <= S; ++i) {
                    layerElem = hset.annSet->outLayers[i];
/*erray9*/
/*ShowNMatrix(layerElem->yFeaMats[1], 2);*/
                ...
                if (optTrainMode == SEQTM) {
                    ...
                    for (i = 1; i <= S; ++i) {
                        if (optHasFSmooth) {
                            CalcOutLayerBackwardSignal(hset.annSet->outLayers[i], nLoaded, objfunFSmooth, 1);
                            ScaleNMatrix(1.0 - FSmoothH, nLoaded, hset.annSet->outLayers[i]->nodeNum, hset.annSet->outLayers[i]->yFeaMats[1]);	/* cz277 - many */
                        }
                        else {
                            SetNMatrix(0.0, hset.annSet->outLayers[i]->yFeaMats[1], nLoaded);	/* cz277 - many */
                        }
#ifdef CUDA
                        SyncNMatrixDev2Host(hset.annSet->outLayers[i]->yFeaMats[1]);	/* cz277 - many */
#endif
                    }
                    if (procNumLats) {
...
#ifdef CUDA
                    for (i = 1; i <= S; ++i) {
                        SyncNMatrixHost2Dev(hset.annSet->outLayers[i]->yFeaMats[1]);	/* cz277 - many */
                    }
#endif
                    if (optFrameReject) {
        ...
        if (sampCnt != 0) {
            ...
            UpdateANNParamSGD(hset.annSet, retNegLR, momentum, weightDecay, sampCnt);
            edClock = clock();
            fbPropClock += edClock - stClock;
            /* update updtCnt */
            ++updtCnt;
    ...
    ResetRplParts(hset.annSet);	/* cz277 - xform */
    /* update the target penalties */
...
void UtterLevelHVProcess(void) {
    ...
    while (!finish) {
        sampCnt = 0;
        uttCnt = 1;
        uttElem = GetCurUttElem(cacheHV[1]);	/* cz277 - xform */
        InstallRplParts(&hset, uttElem);    /* cz277 - xform */
        /* get current utterance info */
	if (optTrainMode == SEQTM) {
            /*uttElem = GetCurUttElem(cacheHV[1]);*/	/* cz277 - xform */
        ...
        while ((!finish) && uttCnt > 0) {
            ...
            for (i = 1; i <= S; ++i) {
                ...
                /* convert posteriors to llr */
                if ((showObjFunKind & MLOF) || (optTrainMode == SEQTM)) {
                    ApplyLogTrans(layerElem->yFeaMats[1], nLoaded, layerElem->nodeNum, hset.annSet->llhMat[i]);	/* cz77 - many */
                    AddNVectorTargetPen(hset.annSet->llhMat[i], hset.annSet->penVec[i], nLoaded, hset.annSet->llhMat[i]);
#ifdef CUDA
                    SyncNMatrixDev2Host(hset.annSet->llhMat[i]);
#endif
...
int main(int argc, char *argv[]) {
    ...
    InitStimu(&modelHeap); //cw564 - stimu
    stClock = 0;
    edClock = 0;
    curEpochNum = 0;
    printf("\n");
    ...
    if (optDoStaticUpdt == FALSE) {
        ...
        if (scriptHV != NULL && GetUpdateIndex() == 0) {
            ...
            STP()->newepoch = 1; //cw564 - stimu -- debug
            UtterLevelHVProcess();
        ...
        /* cz277 - pact */
        if (scriptHV != NULL)
            HError(-1, "HNTrainSGD: Static update will not use the held-out scp");
        if (!(GetDefaultVisitKind() == NONEVK || GetDefaultVisitKind() == UTTVK))
            HError(9999, "HNTrainSGD: Only utterance level cache visiting order is allowed for static update");
