...
void ForwardPropBatch(ANNSet *annSet, int batLen, int *CMDVecPL) {
    ...
    while (curAI != NULL) {
        ...
        /*if (!STP()->acti_surface) {
            int dim = annDef->layerList[0]->nodeNum - STP()->mixnodes - 1;
        ...
        if (!STP()->acti_surface) {
            ...
            for (q = 0; q < STP()->num_phone; ++ q) {
                ...
                for (r = 0; r < dim; ++ r) {
                     ...
                     STP()->mix_surface->matElems[q * dim + r] /= sqrt(PI * 2 * STP()->grid_var);// * STP()->num_phone;
                }
            }
            int kk;
            for (kk = 0; kk < 100; ++ kk) {
                printf("%f ", STP()->mix_surface->matElems[kk]);
            }
            printf("\n");
#ifdef CUDA
        ...
        */
        if (STP()->mix_surface == NULL) {
            int dim = annDef->layerList[0]->nodeNum - STP()->mixnodes - 1;
            STP()->mix_surface = CreateNMatrix(STP()->heap, STP()->num_phone, dim);
            //STP()->dact_split = CreateNMatrix(STP()->heap, GetNBatchSamples(), 2);
        }
        if (STP()->weight_norms == NULL) {
            STP()->weight_norms = CreateNVector(STP()->heap, annDef->layerList[0]->nodeNum - STP()->mixnodes - 1);
        }
...
exit(1);*/
                ...
                case SIGMOIDAF:
                    ...
                    //printf("\n");
                    ApplySigmoidActStimuMix(layerElem->yFeaMats[j], batLen, layerElem->nodeNum, layerElem->yFeaMats[j], STP()->mixnodes, STP()->resdnn_var, STP()->mixscaler_var);
                    /*SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                    for (k = 1023; k < 1071; ++k) printf("%f ", layerElem->yFeaMats[j]->matElems[k]);
                    printf("\n");
                    exit(0);
...
exit(1);*/
                ...
                if (i < annDef->layerNum - 1) {
                    ...
                    */
                    int comb_dim = layerElem->nodeNum - STP()->mixnodes - 1;
                    ...
                    printf("\n");
                    */
                    //SyncNMatrixDev2Host(layerElem->actFunInfo.actParmVec[1]);
                    //ShowNVector(layerElem->actFunInfo.actParmVec[1]);
                    //exit(0);
/*
            STP()->grid_row = 32;
            int p,q,r;
...
*/
                    //printf("%d\n", comb_dim);exit(0);
                    CalcMixSurface(STP()->mix_surface, layerElem->actFunInfo.actParmVec[1], comb_dim, STP()->num_phone, STP()->grid_var, NULL);
                    //if (i == annDef->layerNum - 2 && STP()->enable_update_mix && !STP()->enable_update_dnn) {
                    //    FwdCombDNNandMix(STP()->comb_yFeaMat[i], STP()->mix_surface, layerElem->yFeaMats[j], STP()->comb_softmaxSum[i],
                    //            batLen, layerElem->nodeNum, comb_dim, STP()->mixnodes, 0.0);
                    //}
                    //else {
                        FwdCombDNNandMix(STP()->comb_yFeaMat[i], STP()->mix_surface, layerElem->yFeaMats[j], STP()->comb_softmaxSum[i],
                    ...
                    SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                    for (k = 0; k < batLen; ++ k) {
                        printf("%f ", layerElem->yFeaMats[j]->matElems[k * 1071 + 1070]);
                    }
                    printf("\n");
                    ...
                    SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                    printf("\nmagicframe1\n");
                    for (k = 1024; k < 1024 + 46; ++ k) {
                        printf("%f ", layerElem->yFeaMats[j]->matElems[0 * 1071 + k]);
                    }
                    printf("\n");
                    SyncNVectorDev2Host(STP()->comb_softmaxSum[i]);
                    ShowNVector(STP()->comb_softmaxSum[i]);
                    printf("\n");
                    */
                    /*
                    ...
                    printf("\n");
                    */
                    //ShowNMatrix(STP()->comb_yFeaMat[i],2);
                    //exit(0);
                ...
                if (strcmp(STP()->visual_fn, "") && i < annDef->layerNum - 1) {
                    ...
                    //if (!STP()->enable_update_dnn)
                    //    FwdCombDNNandMix(STP()->comb_yFeaMat[i], STP()->mix_surface, layerElem->yFeaMats[j], STP()->comb_softmaxSum[i],
                    //            batLen, layerElem->nodeNum, comb_dim, STP()->mixnodes, 0);
                    SyncNMatrixDev2Host(STP()->comb_yFeaMat[i]);
#endif
                    int kk, jj;
                    for (kk = 0; kk < batLen; ++ kk) {
                        fprintf(tmp_f, "%s", STP()->phone_names[(int)(STP()->phoneidx_vec->vecElems[kk])]);
                        for (jj = 0; jj < layerElem->nodeNum - STP()->mixnodes - 1; ++ jj) {
                            //fprintf(tmp_f, " %f", STP()->comb_yFeaMat[i]->matElems[kk * (layerElem->nodeNum - STP()->mixnodes - 1) + jj] * STP()->weight_norms->vecElems[jj]);
                            fprintf(tmp_f, " %f", STP()->comb_yFeaMat[i]->matElems[kk * (layerElem->nodeNum - STP()->mixnodes - 1) + jj]);
                        }
                        fprintf(tmp_f, "\n");
                    }
                    fclose(tmp_f);
                    //if (!STP()->enable_update_dnn)
                    //    FwdCombDNNandMix(STP()->comb_yFeaMat[i], STP()->mix_surface, layerElem->yFeaMats[j], STP()->comb_softmaxSum[i],
                    //          batLen, layerElem->nodeNum, comb_dim, STP()->mixnodes, 1.0);
        ...
        for (i = annDef->layerNum - 1; i >= 0; --i) {
            ...
            for (j = 1; j <= n; ++j) {
                ...
                if (layerElem->roleKind == OUTRK) {
                    ...
                    CalcMixSurface(STP()->mix_surface, layerElem->actFunInfo.actParmVec[1], annDef->layerList[1]->inputDim, STP()->num_phone, STP()->grid_var, STP()->phonepos);
                    //mix
                    int comb_dim = layerElem->nodeNum - STP()->mixnodes - 1;
                    int kk;
                    ...
                    */
                    ...
                    SplitGradDNNandMix(layerElem->trainInfo->dyFeaMats[j], dyFeaMat, layerElem->yFeaMats[j], STP()->comb_yFeaMat[i], STP()->mix_surface, 
                            batLen, layerElem->nodeNum, comb_dim, STP()->mixscaler, STP()->enable_update_mix, STP()->enable_update_dnn);
                    
                    /*printf("grad:\n");
                    SyncNMatrixDev2Host(layerElem->trainInfo->dyFeaMats[j]);
                    for (k = 0; k < 1071; ++ k) {
                        printf("%f ", layerElem->trainInfo->dyFeaMats[j]->matElems[k]);
                    }
                    printf("\n");
                    printf("s")
                    printf("\n");
...
exit(1);*/
                    ...
                    case PSIGMOIDAF:
                        ...
                        //printf("combdim=%d\n", annDef->layerList[1]->inputDim);exit(0);
                        ClearNVectorSegment(layerElem->trainInfo->gradInfo->actParmVec[1], 0, layerElem->trainInfo->gradInfo->actParmVec[1]->vecLen);
                        if (STP()->enable_update_mix) {
                        //SyncNVectorDev2Host(layerElem->trainInfo->gradInfo->actParmVec[1]);
                        //ShowNVector(layerElem->trainInfo->gradInfo->actParmVec[1]);
                            if (STP()->update_mix_means) {
                                ApplyGradMixMeans(
                                    layerElem->trainInfo->gradInfo->actParmVec[1], 
                                    layerElem->actFunInfo.actParmVec[1],
                                    STP()->comb_dxFeaMat, STP()->mix_surface, layerElem->yFeaMats[j],
                                    batLen, layerElem->nodeNum, annDef->layerList[1]->inputDim, STP()->mixnodes, STP()->mix_mean_l2_penalty, STP()->phonepos);
                                }
                            if (STP()->update_mix_vars) {
                                ApplyGradMixVars(
                                    layerElem->trainInfo->gradInfo->actParmVec[1], 
                                    layerElem->actFunInfo.actParmVec[1],
                                    STP()->comb_dxFeaMat, STP()->mix_surface, layerElem->yFeaMats[j],
                                    batLen, layerElem->nodeNum, annDef->layerList[1]->inputDim, STP()->mixnodes, STP()->mix_var_l2_penalty, STP()->grid_var);
                            }
                            if (STP()->update_mix_rels) {
                                ApplyGradMixRels(
                                    layerElem->trainInfo->gradInfo->actParmVec[1], 
                                    layerElem->actFunInfo.actParmVec[1],
                                    STP()->comb_dxFeaMat, STP()->mix_surface, layerElem->yFeaMats[j],
                                    batLen, layerElem->nodeNum, annDef->layerList[1]->inputDim, STP()->mixnodes, STP()->mix_rel_l2_penalty);
                            }
                        }
                        //SyncNMatrixDev2Host(layerElem->actFunInfo.actParmVec[1]);
                        /*
                        ...
                        SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                        for (kk = 0; kk < 1100; ++ kk) {
                            printf("%f ", layerElem->yFeaMats[j]->matElems[kk]);
                            //printf("%f\n", STP()->comb_dxFeaMat->matElems[kk]);
                            //printf("%f\n", layerElem->actFunInfo.actParmVec[1]->vecElems[kk]);
                        }
                        //SyncNVectorDev2Host(layerElem->trainInfo->gradInfo->actParmVec[1]);
                        //ShowNVector(layerElem->trainInfo->gradInfo->actParmVec[1]);
                        exit(0);
...
exit(1);*/
                    ...
                    case SIGMOIDAF:
                        ...
                        */
                        //cw564 - stimu -- mix
                        ApplyDSigmoidActStimuMix(layerElem->yFeaMats[j], batLen, layerElem->nodeNum, layerElem->yFeaMats[j], comb_dim, STP()->resdnn_var, STP()->mixscaler_var);//mix
                        /*
                        SyncNMatrixDev2Host(layerElem->yFeaMats[j]);
                        for (k = 1023; k < 1071; ++ k) {
                            printf("%f ", layerElem->yFeaMats[j]->matElems[k]);
                ...
                int raw_dim = layerElem->inputDim + STP()->mixnodes + 1;
                if (STP()->comb_dxFeaMat == NULL) {
                    STP()->comb_dxFeaMat = CreateNMatrix(STP()->heap, GetNBatchSamples(), comb_dim);
                }
                ClearNMatrixSegment(STP()->comb_dxFeaMat, 0, GetNBatchSamples() * comb_dim);
...
void RandANNLayer(LELink layerElem, int seed, float scale) {
    ...
    int res_dnn_dim = 1024;
    //printf("HNStimu: [Warn] Stimu mix init.\n");
    ...
    default:
        ...
        RandInit(seed);
        //RandNSegmentUniform(-1.0 * r, r, layerElem->wghtMat->rowNum * layerElem->wghtMat->colNum, layerElem->wghtMat->matElems);
        RandNSegmentUniformStimuMix(-1.0 * r, r, layerElem->wghtMat->rowNum, layerElem->wghtMat->colNum, layerElem->wghtMat->matElems, res_dnn_dim);
...
void InitStimu(MemHeap * heap) {
    ...
    */
    ...
    stp.resdnn_var = 9.5; //magic
    stp.mixscaler_var = 100.0; //magic too
    ...
    stp.enable_update_mix = TRUE;
    stp.enable_update_dnn = TRUE;
    stp.init_mix = FALSE;


    stp.mix_mean_l2_penalty = 0.0;
    stp.mix_var_l2_penalty = 0.0;
    stp.mix_rel_l2_penalty = 0.0;

    stp.update_mix_means = FALSE;
    stp.update_mix_vars = FALSE;
    stp.update_mix_rels = FALSE;

    if (nSTParm > 0) {
        if (GetConfFlt(cSTParm, nSTParm, "MIXRELL2PENALTY", &doubleVal)) {
            stp.mix_rel_l2_penalty = doubleVal;
        }
        if (GetConfFlt(cSTParm, nSTParm, "MIXMEANL2PENALTY", &doubleVal)) {
            stp.mix_mean_l2_penalty = doubleVal;
        }
        if (GetConfFlt(cSTParm, nSTParm, "MIXVARL2PENALTY", &doubleVal)) {
            stp.mix_var_l2_penalty = doubleVal;
        }
        if (GetConfBool(cSTParm, nSTParm, "UPDATEMIXREL", &boolVar)) {
            stp.update_mix_rels = boolVar;
        }
        if (GetConfBool(cSTParm, nSTParm, "UPDATEMIXMEAN", &boolVar)) {
            stp.update_mix_means = boolVar;
        }
        if (GetConfBool(cSTParm, nSTParm, "UPDATEMIXVAR", &boolVar)) {
            stp.update_mix_vars = boolVar;
        }
        if (GetConfBool(cSTParm, nSTParm, "INITMIX", &boolVar)) {
            stp.init_mix = boolVar;
        }
        if (GetConfFlt(cSTParm, nSTParm, "MIXSCALERVAR", &doubleVal)) {
            stp.mixscaler_var = doubleVal;
        }
        if (GetConfFlt(cSTParm, nSTParm, "RESDNNVAR", &doubleVal)) {
            stp.resdnn_var = doubleVal;
        }
        if (GetConfBool(cSTParm, nSTParm, "UPDATEMIX", &boolVar)) {
            stp.enable_update_mix = boolVar;
        }
        if (GetConfBool(cSTParm, nSTParm, "UPDATEDNN", &boolVar)) {
            stp.enable_update_dnn = boolVar;
        }
...
void PrintSilPerformance() {
    printf("\t\tSilPrecision=%lf\n", ((float) stp.PosPreSil) / stp.totPreSil);
    printf("\t\tSilRecall=%lf\n", ((float) stp.PosPreSil) / stp.totPosSil);
    stp.PosPreSil = 0;
    stp.totPreSil = 0;
    stp.totPosSil = 0;
}
