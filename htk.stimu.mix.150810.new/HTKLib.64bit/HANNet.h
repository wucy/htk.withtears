...
typedef struct _StimuParam {
    ...
    float lhuc_penalty;
    int lhuc_penalty_type;

    float lhuc_range_var;
    float lhuc_dist_var;

    NMatrix *d_lhuc;
    NMatrix *pos_dist;
    
    double accumLHUCRegVal;
    NVector *lhuc_regVal;
    ...
    //mixture -- begin
    float mixscaler;
    float dnnscaler; // dnnscaler = 1 - mixscaler

    int mixnodes;
    NMatrix *comb_yFeaMat[10];
    NVector *comb_softmaxSum[10];
    NMatrix *comb_dxFeaMat;

    NMatrix *mix_surface; //similar to acti_surface, but divided by maxval
    ...
    Boolean enable_update_mix;
    Boolean enable_update_dnn;

    Boolean update_mix_means;
    Boolean update_mix_vars;
    Boolean update_mix_rels;

    float resdnn_var;
    float mixscaler_var;


    float mix_mean_l2_penalty;
    float mix_var_l2_penalty;
    float mix_rel_l2_penalty;
...
extern void PrintLHUCRegVal(int num_samples, double ce, double lhuc_penalty);
extern void ResetStimu();
extern void PrintSilPerformance();
//cw564 - stimu -- end
