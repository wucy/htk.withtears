...
void ApplyAffineAct(NMatrix *srcMat, int row, int col, NVector *scaleVec, NVector *shiftVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && dstMat->colNum == col && scaleVec->vecLen == col && shiftVec->vecLen == col))
            HError(9999, "ApplyAffineAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyAffineAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDAffineAct(NMatrix *srcMat, int row, int col, NVector *scaleVec, NVector *shiftVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && dstMat->colNum == col && scaleVec->vecLen == col && shiftVec->vecLen == col))
            HError(9999, "ApplyDAffineAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDAffineAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrAffineAct(NMatrix *errMat, NMatrix *actMat, int row, int col, NVector *scaleVec, NVector *shiftVec, Boolean accFlag, NVector *dScaleVec, NVector *dShiftVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && actMat->colNum == col && scaleVec->vecLen == col && shiftVec->vecLen == col && dScaleVec->vecLen == col && dShiftVec->vecLen == col))
            HError(9999, "ApplyDTrAffineAct: Input column number incompatible");
    }
#ifdef CUDA
...
void AccMeanNVector(NMatrix *valMat, int row, int col, NFloat tSamp, NVector *meanVec) {
    ...
    if (trace & T_DIM) {
        if (!(valMat->colNum == col && meanVec->vecLen == col))
            HError(9999, "ApplyInitMeanAMVNAct: Input column number incompatible");
        /*if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
...
void AccVarianceNVector(NMatrix *srcMat, int row, int col, NFloat tSamp, NVector *meanVec, NVector *varVec) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && meanVec->vecLen == col && varVec->vecLen == col))
            HError(9999, "AccVarianceNVector: Input column number incompatible");
        /*if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDAMVNAct: Input row number out of range");*/
    }
#ifdef CUDA
...
void ApplyReLUAct(NMatrix *srcMat, int row, int col, NFloat scale, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && dstMat->colNum == col))
            HError(9999, "ApplyReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDReLUAct(NMatrix *srcMat, int row, int col, NFloat scale, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && dstMat->colNum == col))
            HError(9999, "ApplyDReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyLHUCSigmoidAct(NMatrix *srcMat, int row, int col, NVector *roleVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && roleVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyLHUCSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyLHUCSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDLHUCSigmoidAct(NMatrix *srcMat, int row, int col, NVector *roleVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && roleVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyDLHUCSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDLHUCSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrLHUCSigmoidAct(NMatrix *errMat, NMatrix *actMat, int row, int col, NVector *roleVec, Boolean accFlag, NVector *dRoleVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && actMat->colNum == col && roleVec->vecLen == col && dRoleVec->vecLen == col))
            HError(9999, "ApplyTrLHUCSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= errMat->rowNum && row <= actMat->rowNum))
            HError(9999, "ApplyTrLHUCSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyPSigmoidAct(NMatrix *srcMat, int row, int col, NVector *etaVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && etaVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyPSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyPSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDPSigmoidAct(NMatrix *srcMat, int row, int col, NVector *etaVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && etaVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyDPSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDPSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrPSigmoidAct(NMatrix *errMat, NMatrix *srcMat, NVector *etaVec, int row, int col, Boolean accFlag, NVector *dEtaVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && srcMat->colNum == col && etaVec->vecLen == col && dEtaVec->vecLen == col))
            HError(9999, "ApplyTrPSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= errMat->rowNum))
            HError(9999, "ApplyTrPSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyParmSigmoidAct(NMatrix *srcMat, int row, int col, NVector *etaVec, NVector *gammaVec, NVector *thetaVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && etaVec->vecLen == col && gammaVec->vecLen == col && thetaVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyParmSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyParmCSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDParmSigmoidAct(NMatrix *srcMat, int row, int col, NVector *etaVec, NVector *gammaVec, NVector *thetaVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && etaVec->vecLen == col && gammaVec->vecLen == col && thetaVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyDParmSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDParmSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrParmSigmoidAct(NMatrix *errMat, NMatrix *inpMat, int row, int col, NVector *etaVec, NVector *gammaVec, NVector *thetaVec, Boolean accFlag, NVector *dEtaVec, NVector *dGammaVec, NVector *dThetaVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && inpMat->colNum == col && etaVec->vecLen == col && gammaVec->vecLen == col && thetaVec->vecLen == col && dEtaVec->vecLen == col && dGammaVec->vecLen == col && dThetaVec->vecLen == col))
            HError(9999, "ApplyTrParmSigmoidAct: Input column number incompatible");
        if (!(row > 0 && row <= inpMat->rowNum && row <= errMat->rowNum))
            HError(9999, "ApplyTrParmSigmoidAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyParmReLUAct(NMatrix *srcMat, int row, int col, NVector *posVec, NVector *negVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && posVec->vecLen == col && negVec->vecLen && dstMat->colNum == col))
            HError(9999, "ApplyParmReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyParmReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDParmReLUAct(NMatrix *inpMat, int row, int col, NVector *posVec, NVector *negVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(inpMat->colNum == col && posVec->vecLen == col && negVec->vecLen && dstMat->colNum == col))
            HError(9999, "ApplyDParmReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= inpMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyDParmReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrParmReLUAct(NMatrix *errMat, NMatrix *inpMat, int row, int col, Boolean accFlag, NVector *dPosVec, NVector *dNegVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && inpMat->colNum == col && dPosVec->vecLen == col && dNegVec->vecLen == col))
            HError(9999, "ApplyTrParmReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= inpMat->rowNum && row <= errMat->rowNum))
            HError(9999, "ApplyTrParmReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyPReLUAct(NMatrix *srcMat, int row, int col, NVector *scaleVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && scaleVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyPReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyPReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyDPReLUAct(NMatrix *srcMat, int row, int col, NVector *scaleVec, NMatrix *dstMat) {
    ...
    if (trace & T_DIM) {
        if (!(srcMat->colNum == col && scaleVec->vecLen == col && dstMat->colNum == col))
            HError(9999, "ApplyDPReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= dstMat->rowNum))
            HError(9999, "ApplyPDReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void ApplyTrPReLUAct(NMatrix *errMat, NMatrix *srcMat, int row, int col, NVector *scaleVec, Boolean accFlag, NVector *dScaleVec) {
    ...
    if (trace & T_DIM) {
        if (!(errMat->colNum == col && srcMat->colNum == col && scaleVec->vecLen == col && dScaleVec->vecLen == col))
            HError(9999, "ApplyDPReLUAct: Input column number incompatible");
        if (!(row > 0 && row <= srcMat->rowNum && row <= errMat->rowNum))
            HError(9999, "ApplyDPReLUAct: Input row number out of range");
    }
#ifdef CUDA
...
void RandNSegmentGaussian(NFloat mu, NFloat sigma, int segLen, NFloat *segPtr) {
    ...
    if (sigma <= 0.0)
        HError(9999, "RandNSegmentGauss: Standard deviation need to be positive");
    for (i = 0; i < segLen; ++i) {
...
void MixValidate(NVector *act, int num_phones, int len) {
#ifdef CUDA
    MixValidateCUDA(act->devElems, num_phones, len);
#endif
}


void ApplyGradMixMeans(NVector* dact, NVector* act, NMatrix* dyFeaMat, NMatrix* mix_surface, NMatrix* raw_y, int batLen, int nodeNum, int comb_dim, int mixnodes, NFloat mix_mean_l2_penalty, NMatrix* phonepos) {
#ifdef CUDA
   //printf("GEGEDADA\n"); exit(0);
   ApplyGradMixMeansCUDA(dact->devElems, act->devElems, dyFeaMat->devElems, mix_surface->devElems, raw_y->devElems, batLen, nodeNum, comb_dim, mixnodes, mix_mean_l2_penalty, phonepos->devElems);
#endif
}

void ApplyGradMixVars(NVector* dact, NVector* act, NMatrix* dyFeaMat, NMatrix* mix_surface, NMatrix* raw_y, int batLen, int nodeNum, int comb_dim, int mixnodes, NFloat mix_var_l2_penalty, NFloat gridvar) {
#ifdef CUDA
   //printf("GEGEDADA\n"); exit(0);
   ApplyGradMixVarsCUDA(dact->devElems, act->devElems, dyFeaMat->devElems, mix_surface->devElems, raw_y->devElems, batLen, nodeNum, comb_dim, mixnodes, mix_var_l2_penalty, gridvar);
#endif
}

void ApplyGradMixRels(NVector* dact, NVector* act, NMatrix* dyFeaMat, NMatrix* mix_surface, NMatrix* raw_y, int batLen, int nodeNum, int comb_dim, int mixnodes, NFloat mix_rel_l2_penalty) {
#ifdef CUDA
   //printf("GEGEDADA\n"); exit(0);
   ApplyGradMixRelsCUDA(dact->devElems, act->devElems, dyFeaMat->devElems, mix_surface->devElems, raw_y->devElems, batLen, nodeNum, comb_dim, mixnodes, mix_rel_l2_penalty);
#endif
}


inline float my_tanh(float raw) {
    float floatVal = raw;
    CHKNFLTEXPE(floatVal)
    floatVal = exp(floatVal);
    return (floatVal - 1.0 / floatVal) / (floatVal + 1.0 / floatVal);
}
inline float my_exp(float raw) {
    CHKNFLTEXPE(raw)
    return exp(raw);
}

void CalcMixSurface(NMatrix* mix_surface, NVector* actParmVec, int comb_dim, int mixnodes, float grid_var, NMatrix * phonepos) {
#ifdef CUDA
    CalcMixSurfaceCUDA(mix_surface->devElems, actParmVec->devElems, comb_dim, mixnodes, grid_var);
#else
    int grid_one_dim = (int)sqrt(comb_dim);
    int i, q, r, row_no, col_no;
    float x, y, dissq;
    float diff1, diff2, diff;
    float s1, s2, rou, s1s2, one_min_rousqr, norm;
    for (i = 0; i < comb_dim * mixnodes; ++ i) {
        q = i / comb_dim;
        r = i % comb_dim;
        ...
        s1 = my_exp(actParmVec->vecElems[q * 6 + 2]);
        s2 = my_exp(actParmVec->vecElems[q * 6 + 5]);
        rou = my_tanh(actParmVec->vecElems[q * 6 + 3]);
        
        //s1 = actParmVec->vecElems[q * 6 + 2];
        //s2 = actParmVec->vecElems[q * 6 + 5];
        //rou = actParmVec->vecElems[q * 6 + 3];
        
        s1s2 = s1 * s2;
        one_min_rousqr = 1 - rou * rou;
        norm = 1.0 / (PI * 2.0 * sqrt(one_min_rousqr) * s1s2);
        diff1 = x - actParmVec->vecElems[q * 6];
        diff2 = y - actParmVec->vecElems[q * 6 + 1];
        diff = -(diff1 * diff1 / s1 / s1 + diff2 * diff2 / s2 / s2 - 2 * rou * diff1 * diff2 / s1s2) / (2 * one_min_rousqr);
        mix_surface->matElems[i] = exp(diff) * norm;
    }
    //SyncNMatrixHost2Dev(mix_surface);
#endif
}

void RandNSegmentUniformStimuMix(NFloat lower, NFloat upper, int row, int col, NFloat *segPtr, int res_dnn_dim) {
    int i,j;
    NFloat range;
    ...
    if (row > 2000) {
        ...
        printf("HNStimu: [Warn] Stimu mix init, row=%d, col=%d.\n", row, col);
        NFloat resdnn_range = range / 40.0;
        NFloat resdnn_lower = lower / 40.0;
        for (j = 0; j < row; ++ j) {
            if (j < res_dnn_dim) {
                for (i = 0; i < col; ++ i) {
                    //segPtr[j * col + i] = RandomValue() * resdnn_range + resdnn_lower;
                    segPtr[j * col + i] = RandomValue() * 0.0;
                }
            }
            else if (j >= res_dnn_dim && j < row - 1) {
                for (i = 0; i < col; ++ i) {
                    segPtr[j * col + i] = RandomValue() * range + lower;
                }
            }
            else if (j == row - 1) {
                for (i = 0; i < col; ++ i) {
                ...
                HError(9999, "RandNSegmentStimuMix: not reach");
            }
        }
    }
}

void ApplyDSigmoidActStimuMix(NMatrix *srcMat, int row, int col, NMatrix *dstMat, int comb_dim, float resdnn_var, float mixscaler_var) {
#ifdef CUDA
    NFloat *srcMat_elems = srcMat->devElems;
    NFloat *dstMat_elems = dstMat->devElems;
    ApplyDSigmoidActStimuMixCUDA(srcMat_elems, row * col, dstMat_elems, col, comb_dim, resdnn_var, mixscaler_var);
#else
    HError(9999, "ApplyDSigmoidActStimuMix: not support CPU.");
#endif
}


void SplitGradDNNandMix(NMatrix *dest_dy, NMatrix *comb_dy, NMatrix *raw_y, NMatrix *comb_y, NMatrix *mix_surface, int batLen, int raw_dim, int comb_dim, float mixscaler, Boolean enable_update_mix, Boolean enable_update_dnn) {
#ifdef CUDA
    NFloat *dest_dy_elems = dest_dy->devElems;
    NFloat *comb_dy_elems = comb_dy->devElems;
    NFloat *mix_surface_elems = mix_surface->devElems;
    NFloat *raw_y_elems = raw_y->devElems;
    NFloat *comb_y_elems = comb_y->devElems;
    SplitGradDNNandMixCUDA(dest_dy_elems, comb_dy_elems, raw_y_elems, comb_y_elems, mix_surface_elems, batLen, raw_dim, comb_dim, mixscaler, enable_update_mix, enable_update_dnn);
#else
    HError(9999, "SplitGradDNNandMix: not support CPU.");
#endif
}

void ApplySigmoidActStimuMix(NMatrix *srcMat, int row, int col, NMatrix *dstMat, int mixnodes, float resdnn_var, float mixscaler_var) {
#ifdef CUDA
    NFloat *srcMat_elems = srcMat->devElems;
    NFloat *dstMat_elems = dstMat->devElems;
    ApplySigmoidActStimuMixCUDA(srcMat_elems, row * col, dstMat_elems, mixnodes, col, resdnn_var, mixscaler_var);
#else
    //HError(9999, "ApplySigmoidActStimuMix: not support CPU.");
    int pos;
    int raw_col = col;
    NFloat COMBLITTLE = 1e-6;
    NFloat floatVal;
    NFloat *srcPtr = srcMat->matElems;
    NFloat *dstPtr = dstMat->matElems;
    for (pos = 0; pos < row * col; ++ pos) {
...
void FwdCombDNNandMix(NMatrix *comb_yFeaMat, NMatrix *mix_surface, NMatrix *raw_yFeaMat, NVector *comb_softmaxSum, int batLen, int raw_nodeNum, int comb_nodeNum, int mixnodes, float mixscaler) {
#ifdef CUDA
    NFloat *comb_yFeaMat_elems = comb_yFeaMat->devElems;
    NFloat *mix_surface_elems = mix_surface->devElems;
    NFloat *raw_yFeaMat_elems = raw_yFeaMat->devElems;
    NFloat *comb_softmaxSum_elems = comb_softmaxSum->devElems;
    FwdCombDNNandMixCUDA(comb_yFeaMat_elems, mix_surface_elems, raw_yFeaMat_elems, comb_softmaxSum_elems, batLen, raw_nodeNum, comb_nodeNum, mixnodes, mixscaler);
#else
    //HError(9999, "FwdCombDNNandMix: not support CPU.");
    int i, j, k;
    NFloat *raw_y = raw_yFeaMat->matElems;
    NFloat *comb_y = comb_yFeaMat->matElems;
    NFloat *mix_surf = mix_surface->matElems;
    NFloat tmp;
    for (i = 0; i < batLen; ++ i) {
        int first_mix_ptr = i * raw_nodeNum + comb_nodeNum;
        NFloat maxVal = raw_y[first_mix_ptr];
        for (j = first_mix_ptr + 1; j < first_mix_ptr + mixnodes; ++ j) {
            if (maxVal < raw_y[j]) {
                maxVal = raw_y[j];
            }
        }
        NFloat sumVal = 0;
        for (j = first_mix_ptr; j < first_mix_ptr + mixnodes; ++ j) {
            raw_y[j] -= maxVal;
            tmp = raw_y[j];
            CHKNFLTEXPE(tmp)
            raw_y[j] = exp(tmp);
        ...
        int first_ptr = i * raw_nodeNum;
        NFloat scaler = raw_y[first_mix_ptr + mixnodes];
        for (j = 0; j < comb_nodeNum; ++ j) { 
            int dst_ptr = j + comb_nodeNum * i;
            comb_y[dst_ptr] = 0;
            for (k = 0; k < mixnodes; ++ k) {
                comb_y[dst_ptr] += mix_surf[k * comb_nodeNum + j] * raw_y[first_mix_ptr + k];
            }
            comb_y[dst_ptr] *= scaler;
            comb_y[dst_ptr] += mixscaler * raw_y[first_ptr + j];
        }
    }
#endif
}

void ApplyActLHUCPenalty(NVector * actscaler, NVector * dactscaler, int nodeNum, float lhuc_penalty, Boolean accFlag) {
#ifdef CUDA
    //if (!accFlag) {
    //    return;
    //}
    NFloat *actscaler_elems = actscaler->devElems;
    NFloat *dactscaler_elems = dactscaler->devElems;
    ApplyActLHUCPenaltyCUDA(actscaler_elems, dactscaler_elems, nodeNum, lhuc_penalty);
#else
    HError(9999, "ApplyActLHUCPenalty: not support CPU.");
#endif
}

void ApplyActLHUCPenaltyLocalSoft(NVector * lhuc, NVector * dest_d_lhuc, NMatrix * d_lhuc_surface, int nodeNum, int batLen, float lhuc_penalty, NVector * penalty_val) {
#ifdef CUDA
    NFloat * lhuc_elems = lhuc->devElems;
    NFloat * dest_d_lhuc_elems = dest_d_lhuc->devElems;
    NFloat * d_lhuc_surface_elems = d_lhuc_surface->devElems;
    NFloat * penalty_val_elems = penalty_val->devElems;
    //printf("inmath=%f\n", lhuc_penalty);
    ApplyActLHUCPenaltyLocalSoftCUDA(lhuc_elems, dest_d_lhuc_elems, d_lhuc_surface_elems, nodeNum, batLen, lhuc_penalty, penalty_val_elems);
#else
    HError(9999, "ApplyActLHUCPenaltyLocalSoft: not support CPU.");
#endif
}

void ApplyActLHUCPenaltySoft(NMatrix * d_lhuc, NVector * lhuc, NVector * dest_d_lhuc, NVector * phoneidx, NMatrix * pos_dist, float lhuc_range_var, float lhuc_dist_var, int nodeNum, int batLen, float lhuc_penalty, Boolean accFlag, NVector * penal_val) {
#ifdef CUDA
    //if (!accFlag) {
    //    return;
    //}
    NFloat * d_lhuc_elems = d_lhuc->devElems;
    NFloat * lhuc_elems = lhuc->devElems;
    NFloat * dest_d_lhuc_elems = dest_d_lhuc->devElems;
    NFloat * phoneidx_elems = phoneidx->devElems;
    NFloat * pos_dist_elems = pos_dist->devElems;
    NFloat * penal_val_elems = penal_val->devElems;
    //ShowNVector(lhuc);
    //exit(0);
    ApplyActLHUCPenaltySoftCUDA(d_lhuc_elems, lhuc_elems, dest_d_lhuc_elems, phoneidx_elems, pos_dist_elems, lhuc_range_var, lhuc_dist_var, nodeNum, batLen, lhuc_penalty, penal_val_elems);
    //ShowNVector(penal_val);
#else
    HError(9999, "ApplyActLHUCPenalty: not support CPU.");
#endif
}
...
void ApplyActStimuPenalty(NMatrix * stimu_dy, NMatrix * y, NVector * actscaler, NVector * dactscaler, int batLen, int nodeNum, float stimu_penalty, Boolean accFlag) {
#ifdef CUDA
    //if (!accFlag) {
    //    return;
    //}
    NFloat * stimu_dy_elems = stimu_dy->devElems;
    NFloat * y_elems = y->devElems;
    NFloat * actscaler_elems = actscaler->devElems;
    NFloat * dactscaler_elems = dactscaler->devElems;
    ApplyActStimuPenaltyCUDA(stimu_dy_elems, y_elems, actscaler_elems, dactscaler_elems, batLen, nodeNum, stimu_penalty);
#else
    HError(9999, "ApplyActStimuPenalty: not support CPU.");
#endif
}

void StimuGrad(NMatrix * srcPtr, int row, int col, NMatrix * grid_suface, int num_phone, NVector * phone_vec, NMatrix *dstPtr, NVector * weight_norm) {
#ifdef CUDA
    NFloat * srcPtr_elem = srcPtr->devElems;
    NFloat * grid_suface_elem = grid_suface->devElems;
    NFloat * phone_vec_elem = phone_vec->devElems;
    NFloat * dstPtr_elem = dstPtr->devElems;
    NFloat * weight_norm_elem = weight_norm->devElems;
    if (weight_norm != NULL) {
        weight_norm_elem = weight_norm->devElems;
    }
    //HError(9999, "%d %d\n", grid_suface->rowNum, grid_suface->colNum);
    
    //HError(1009, "");
    //SyncNVectorDev2Host(phone_vec);
    ...
    /*for (kk = 0; kk < 32; kk ++)
    {
        for (mm = 0; mm < 32; mm ++)
        {
            printf("%f ", grid_suface->matElems[(kk * 32 + mm) * 46 + 2]);
        }
        printf("\n");
    }*/
    ...
    for (kk = 0; kk < row; ++ kk)
    {
        printf("%d ", (int)(phone_vec->vecElems[kk]));
    }
    printf("\n");
    exit(0);
    */
    //printf("%d\n", num_phone);
    StimuGradCUDA(srcPtr_elem, row, col, grid_suface_elem, num_phone, phone_vec_elem, dstPtr_elem, weight_norm_elem);
#else
    HError(9999, "StimuGrad: not support CPU");
#endif
}

void CalcWeightNorms(NVector * weight_norms, NMatrix * weights, int input_dim, int output_dim) {
#ifdef CUDA
    NFloat * weight_norms_elems = weight_norms->devElems;
    NFloat * weights_elems = weights->devElems;
    CalcWeightNormsCUDA(weight_norms->devElems, weights->devElems, input_dim, output_dim);
#else
    HError(9999, "CalcWeightNorms: not support CPU");
#endif
}

void CalcSumActi(NVector * sum_xFeaMat, NMatrix * xFeaMats, NVector * weight_norms, int batLen, int nodeNum) {
#ifdef CUDA
    NFloat * sum_xFeaMat_elems = sum_xFeaMat->devElems;
    NFloat * xFeaMats_elems = xFeaMats->devElems;
    NFloat * weight_norms_elems = weight_norms->devElems;
    
    CalcSumActiCUDA(sum_xFeaMat_elems, xFeaMats_elems, weight_norms_elems, batLen, nodeNum);

#else
    HError(9999, "CalcSumActi: not support CPU");
#endif
}


void CalcStimuKL(NVector * klvec, NMatrix * yFeaMats, NVector * weight_norms, NVector * sum_yFeaMats, NMatrix * acti_surface, NVector * phoneidx_vec, int batLen, int nodeNum, int num_phone) {
#ifdef CUDA
    NFloat *klvec_elems = klvec->devElems;
    NFloat *yFeaMats_elems = yFeaMats->devElems;
    NFloat *weight_norms_elems = weight_norms->devElems;
    NFloat *sum_yFeaMats_elems = sum_yFeaMats->devElems;
    NFloat *acti_surface_elems = acti_surface->devElems;
    NFloat *phoneidx_vec_elems = phoneidx_vec->devElems;
    
    CalcStimuKLCUDA(klvec_elems, yFeaMats_elems, weight_norms_elems, sum_yFeaMats_elems, acti_surface_elems, phoneidx_vec_elems, batLen, nodeNum, num_phone);

#else
    HError(9999, "CalcStimuKL: not support GPU");
#endif
}
