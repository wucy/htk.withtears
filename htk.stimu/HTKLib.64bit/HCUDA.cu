...
__global__ void HKern_CalcStimuKL(NFloat *klvec, NFloat *yFeaMats, NFloat *weight_norms, NFloat *sum_yFeaMats, NFloat *acti_surface, NFloat *phoneidx_vec, int batLen, int nodeNum) {
    extern __shared__ NFloat tmpPtr[];
...
void CalcStimuKLCUDA(NFloat *klvec, NFloat *yFeaMats, NFloat *weight_norms, NFloat *sum_yFeaMats, NFloat *acti_surface, NFloat *phoneidx_vec, int batLen, int nodeNum) {
    int sBytes = sizeof(NFloat) * THREADPERBLOCK;
    HKern_CalcStimuKL<<<1, THREADPERBLOCK, sBytes>>>(klvec, yFeaMats, weight_norms, sum_yFeaMats, acti_surface, phoneidx_vec, batLen, nodeNum);
}
...
#endif
    if (status != CUBLAS_STATUS_SUCCESS) {
        HError(9999, "HNBlasTNgemmCUDA: CUBLAS library gemm function failed, code = %d", status);
    }
}
