...
/* cz277 - xform */
static void LoadRplParts(HMMSet *hset, UttElem *uttElem) {
    int i;
    Boolean maskMatch;
    RPLInfo *rplInfo;
    ...
    for (i = 1; i < MAXNUMRPLNMAT; ++i) {
        rplInfo = hset->annSet->rplNMatInfo[i];
        if (rplInfo != NULL && rplInfo->inActive == TRUE) {
            maskMatch = MaskMatch(rplInfo->inRPLMask, curSpkr, uttElem->uttName);
            if (maskMatch == FALSE) {
                HError(9999, "LoadRplParts: Input mask %s does not match the name %s", rplInfo->inRPLMask, uttElem->uttName);
            }
            MakeRplNMatName(curSpkr, i, rplInfo->cacheInSpkr);
            MakeFN(rplInfo->cacheInSpkr, NULL, rplInfo->inRPLExt, curMacroName);
            MakeFN(curSpkr, NULL, rplInfo->inRPLExt, curFN);
            uttElem->rplNMatHost[i] = LoadOneRplNMat(hset, rplInfo->inRPLDir, curFN, curMacroName); 
            if (uttElem->rplNMatHost[i] == NULL) 
                HError(9999, "LoadRplParts: %s does not have replaceable matrix %d", uttElem->uttName, i);
            if (uttElem->rplNMatHost[i]->rowNum != rplInfo->rplNMat->rowNum || uttElem->rplNMatHost[i]->colNum != rplInfo->rplNMat->colNum) 
                HError(9999, "LoadRplParts: %s does not have consistent replaceable matrix %d", uttElem->uttName, i);
        }
    }
    for (i = 1; i < MAXNUMRPLNVEC; ++i) {
        rplInfo = hset->annSet->rplNVecInfo[i];
        if (rplInfo != NULL && rplInfo->inActive == TRUE) {
            maskMatch = MaskMatch(rplInfo->inRPLMask, curSpkr, uttElem->uttName);
            if (maskMatch == FALSE) {
                HError(9999, "LoadRplParts: Input mask %s does not match the name %s", rplInfo->inRPLMask, uttElem->uttName);
            }
            MakeRplNVecName(curSpkr, i, rplInfo->cacheInSpkr);
            MakeFN(rplInfo->cacheInSpkr, NULL, rplInfo->inRPLExt, curMacroName);
            MakeFN(curSpkr, NULL, rplInfo->inRPLExt, curFN);
            uttElem->rplNVecHost[i] = LoadOneRplNVec(hset, rplInfo->inRPLDir, curFN, curMacroName);
            if (uttElem->rplNVecHost[i] == NULL)
                HError(9999, "LoadRplParts: %s does not have replaceable vector %d", uttElem->uttName, i);
            if (uttElem->rplNVecHost[i]->vecLen != rplInfo->rplNVec->vecLen)
                HError(9999, "LoadRplParts: %s does not have consistent replaceable vector %d", uttElem->uttName, i);
        }
    }
}

/* cz277 - xform */
void SaveAllRplParts(HMMSet *hset, FILE *script) {
    int i;
    Boolean maskMatch;
    RPLInfo *rplInfo;
    ...
    while (GetNextScpWord(script, uttName) != NULL) {
        ...
        for (i = 1; i < MAXNUMRPLNMAT; ++i) {
            rplInfo = hset->annSet->rplNMatInfo[i]; 
            if (rplInfo != NULL && rplInfo->inActive == TRUE) {
                maskMatch = MaskMatch(rplInfo->inRPLMask, rplInfo->curInSpkr, uttName);
                if (maskMatch == FALSE)
                    HError(9999, "SaveRplParts: Input mask %s does not match the name %s", rplInfo->inRPLMask, uttName);
                if (strcmp(rplInfo->cacheInSpkr, rplInfo->curInSpkr) == 0) {
                    MakeRplNMatName(rplInfo->curInSpkr, i, curMacroName);
                    hostMat = LoadOneRplNMat(hset, NULL, NULL, curMacroName);
                    MakeFN(rplInfo->curInSpkr, rplInfo->outRPLDir, rplInfo->outRPLExt, curFN);
                    SaveOneRplNMat(hset, hostMat, curFN, rplInfo->saveBinary);
                    strcpy(rplInfo->cacheInSpkr, rplInfo->curInSpkr);
                }
            }
        } 
        /* output the replaceable vectors */
        for (i = 1; i < MAXNUMRPLNVEC; ++i) {
            rplInfo = hset->annSet->rplNVecInfo[i];
            if (rplInfo != NULL && rplInfo->inActive == TRUE) {
                maskMatch = MaskMatch(rplInfo->inRPLMask, rplInfo->curInSpkr, uttName);
                if (maskMatch == FALSE)
                    HError(9999, "SaveRplParts: Input mask %s does not match the name %s", rplInfo->inRPLMask, uttName);
                if (strcmp(rplInfo->cacheInSpkr, rplInfo->curInSpkr) == 0) {
                    MakeRplNVecName(rplInfo->curInSpkr, i, curMacroName);
                    hostVec = LoadOneRplNVec(hset, NULL, NULL, curMacroName);
                    MakeFN(rplInfo->curInSpkr, rplInfo->outRPLDir, rplInfo->outRPLExt, curFN);
                    SaveOneRplNVec(hset, hostVec, curFN, rplInfo->saveBinary);
                    strcpy(rplInfo->cacheInSpkr, rplInfo->curInSpkr);
                }
            }
        }
...
/* cz277 - xform */
void InstallRplParts(HMMSet *hset, UttElem *uttElem) {
    int i;
    RPLInfo *rplInfo;
    ...
    for (i = 1; i < MAXNUMRPLNMAT; ++i) {
        rplInfo = hset->annSet->rplNMatInfo[i];
        if (rplInfo != NULL && rplInfo->inActive == TRUE) {
            if (rplInfo->rplNMat->matElems != uttElem->rplNMatHost[i]->matElems) {	/* speaker changed */
                MaskMatch(rplInfo->inRPLMask, curSpkr, uttElem->uttName);
                MakeFN(curSpkr, rplInfo->inRPLDir, rplInfo->inRPLExt, curFN);
                if (trace & T_RPL)
                    printf("Using replaceable matrix %d from %s\n", i, curFN);
#ifdef CUDA
                SyncNMatrixDev2Host(rplInfo->rplNMat);	/* synchrnoise previous GPU updates to CPU */
#endif
                rplInfo->rplNMat->matElems = uttElem->rplNMatHost[i]->matElems;
#ifdef CUDA
                SyncNMatrixHost2Dev(rplInfo->rplNMat);	/* refresh GPU device*/
#endif
                rplInfo->rplNMat->hook = &uttElem->rplNMatHost[i]->hook;	/* cz277 - laf */
            }
        } 
    }
    for (i = 1; i < MAXNUMRPLNVEC; ++i) {
        rplInfo = hset->annSet->rplNVecInfo[i];
        if (rplInfo != NULL && rplInfo->inActive == TRUE) {
            if (rplInfo->rplNVec->vecElems != uttElem->rplNVecHost[i]->vecElems) {	/* speaker changed */
                MaskMatch(rplInfo->inRPLMask, curSpkr, uttElem->uttName);
                MakeFN(curSpkr, rplInfo->inRPLDir, rplInfo->inRPLExt, curFN);
                if (trace & T_RPL)
                    printf("Using replaceable vector %d from %s\n", i, curFN);
#ifdef CUDA
                SyncNVectorDev2Host(rplInfo->rplNVec);	/* synchrnoise previous GPU updates to CPU */
#endif
                rplInfo->rplNVec->vecElems = uttElem->rplNVecHost[i]->vecElems;
#ifdef CUDA
                SyncNVectorHost2Dev(rplInfo->rplNVec);	/* refresh GPU device */
#endif
		rplInfo->rplNVec->hook = &uttElem->rplNVecHost[i]->hook;	/* cz277 - laf */
            }
        }
    }
}
...
static ReturnStatus LoadOneUtt(DataCache *cache, int dstPos) {
    ...
    /* cz277 - xform */
    for (i = 1; i < MAXNUMRPLNMAT; ++i) {
        uttElem->rplNMatHost[i] = NULL;
    }
    for (i = 1; i < MAXNUMRPLNVEC; ++i) {
        uttElem->rplNVecHost[i] = NULL;
    }
    LoadRplParts(cache->hmmSet, uttElem);
    ...
    if (cache->labelInfo != NULL) {
        ...
        if ((cache->labelInfo->labelKind & LABLK) != 0) { /* load lab files */
            ...
            for (llink = trans->head->head->succ; llink->succ != NULL; llink = llink->succ) {
                ...
                //cw564 - stimu -- begin
                if (STP()->oidx2phidx == NULL) {
                    STP()->oidx2phidx = (int *) malloc(sizeof(int) * 20000);
                    int i;
                    for (i = 0; i < 20000; ++ i) {
                        STP()->oidx2phidx[i] = -1;
                    }
                ...
                if (STP()->oidx2phidx[streamElem->targetIdx - 1] < 0) {
                    //HError(9999, "DBG:%d %s\n", LabName2STPIdx(llink->labid->name), llink->labid->name);
                    STP()->oidx2phidx[streamElem->targetIdx - 1] = LabName2STPIdx(llink->labid->name);
                }
            ...
            if (i != uttElem->uttLen) {
                HError(9999, "LoadOneUtt: %s Feature and Utterance lengths (%i, %i) do not match", uttElem->uttName, i, uttElem->uttLen);
            }
        }
...
static inline int GetDataFromCache(DataCache *cache, FELink feaElem, int offset) {
    ...
    for (i = 1; i <= n; ++i) {
        ...
        for (j = offset; j < cache->batLen; ++j, feaPtr += extDim) {
            ...
            //printf("uttrid=%d frmid=%d monoid=%d\n", uttIdx, frmIdx, (int)STP()->phoneidx_vec->vecElems[i]);
        }
    }
...
static inline int GetAugFeaFromCache(DataCache *cache, FELink feaElem, int offset) {
    ...
    for (i = 0; i < cache->batLen; ++i) {
        ...
        uttElem = &cache->uttElems[uttIdx];
        if (VectorSize(uttElem->augFeaVec[feaElem->augFeaIdx]) != feaElem->feaDim) {
            HError(9999, "GetAugFeaFromCache: Augmented feature %d dim does't match the feature element dim", feaElem->augFeaIdx);
        }
        for (j = 1; j <= feaElem->ctxMap[0]; ++j, feaPtr += feaElem->feaDim) {
            memcpy(feaPtr, &uttElem->augFeaVec[feaElem->augFeaIdx][1], feaElem->feaDim * sizeof(float));
        }
...
static inline int GetLabelFromCache(DataCache *cache, int tgtDim, NFloat *labBat, int offset) {
    ...
    //cw564 - stimu
    if (STP()->phoneidx_vec == NULL) {
        //STP()->phoneidx_vec = CreateNVector(STP()->heap, 50000);
        //NFloat * tmp = (NFloat *) malloc(sizeof(NFloat) * 50000);
        //STP()->phoneidx_vec->vecElems = tmp;
        STP()->phoneidx_vec = CreateNVector(STP()->heap, GetNBatchSamples());
    }
    ...
    for (i = offset; i < cache->batLen; ++i, labPtr += tgtDim) {
        ...
        uttElem = &cache->uttElems[uttIdx];
        //cw564 - stimu
        //HError(9999, "DBG%d\n", STP()->oidx2phidx[uttElem->labIdxes[frmIdx]]);
        //if (i >= STP()->phoneidx_vec->vecLen) {
        //    HError(9999, "FXXK%d veclen=%d, frmIdx=%d\n", i, STP()->phoneidx_vec->vecLen, frmIdx);
        //}
        STP()->phoneidx_vec->vecElems[i] = STP()->oidx2phidx[uttElem->labIdxes[frmIdx]]; //cw564 - stimu
        //HError(1212, "DBG=%d\n", cache->batLen);
        //cw564 - end
    ...
    *nSamples = 0;
    ...
    for (i = 0; i < nInp; ++i) {
        ...
        if (inpElem[i]->inputKind == INPFEAIK) {
            ...
            HError(9999, "FillAllInpBatch: Can only INPFEAIK and AUGFEAIK from host memory");
        }
#ifdef CUDA
    ...
    if (cache->labMat != NULL) {
        ...
        int kk;
        for (kk = 0; kk < 10; ++ kk) {
            printf("%d ", (int)(STP()->phoneidx_vec->vecElems[kk]));
        }
        printf("\n");exit(0);
        */
        SyncNVectorHost2Dev(STP()->phoneidx_vec);
        //SyncNVectorHost2Dev(STP()->oidx2phidx);
        //cw564 - end
