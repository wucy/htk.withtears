/* ----------------------------------------------------------
   ...
   Copyright (C) 1999-2002 Gunnar Evermann.

  -------------------------------------------------------------
     HLConf:    Calculate Word Confidence Scores from Lattice
  -------------------------------------------------------------
 */
/* test:
...
ge204@parthenope$ ./HLConf -z -A -D -T 1 -L /tmp /home/solveb/hub5/lib/dicts/54khub500v3.lvx.dct che-4316-A_4316Af-che_040814_040934.lat


time dependent posteriors for Luis:

Hub5 evall00 bg lattice HLvx

ge204@tweed$ ./HLConf /home/solveb/hub5/lib/dicts/54khub500v3.lvx.dct /home/solveb/hub5/eval00/work/P3/gi/test_bgint_eval00.vtln-gi.sw1-4683-B.1/lattices/sw1-4683-B_4683Bx-sw1_013028_013197.lat

Hub5 eval00 fgintcat JRlx

ge204@tweed$ ./HLConf /home/solveb/hub5/lib/dicts/54khub500v3.hv.dct /home/solveb/hub5/eval00/work/P4a/gi/test_lm12_10_eval00.vtln-gi.sw1-4683-B.1/lattices/sw1-4683-B_4683Bx-sw1_013028_013197.lat


WSJ tg lattice JRlx/JLRebuild (lfu20)

ge204@tweed$ ./HLConf /home/solveg/lfu20/wsj/litter/lib/dicts/5kcnvp.jrlx.dct /home/solveg/lfu20/wsj/litter/hmm128/test_tg-boc-lm5_csrnab1_dt_s3.6/lattices.1/4rkc0204.lat

ge204@tweed$ ./HLConf -I /tmp/a.mlf /home/shark2/wsjlib/limsi_dicts/5kcnvp.lvx.dct /home/solveg/lfu20/wsj/litter/hmm128/test_bg-boc-lm5_csrnab1_dt_s3.6/lattices/4rkc0204.lat 


real run on lfu20's lattices

ge204@tweed$ ./HLConf -a 0.05555555555555555 -s 1.0 -p 2.777777777777778 -I a.mlf a.dct 4rkc0204.lat
*/
char *hlconf_version = "!HVER!HLConf:   3.0 [GE 25/09/00]";
char *hlconf_sccs_id = "$Id: HLConf.c,v 1.7 2002/03/05 17:46:24 ge204 Exp $";
   ...
   if (nParm>0){
      ...
      if (GetConfStr(cParm,nParm,"CONFMETHOD",buf)) {
         if (strcmp (buf, "GEOMEAN") == 0)
            ...
            HError (9999, "HLConf: unknown CONFMETHOD");
      }
      ...
      if (GetConfFlt(cParm, nParm, "SCALELATSCORE", &f)) {
          ...
          if (latScoreScale <= 0.0)
              HError(9999, "HLConf: SCALELATSCORE must be positive");
      }
...
int main(int argc, char *argv[])
{
   ...
   if(InitShell (argc, argv, hlconf_version, hlconf_sccs_id) < SUCCESS)
      HError (4000, "HLConf: InitShell failed");
   ...
   InitAudio();
   if (InitParm()<SUCCESS) 
      HError(4000,"HLConf: InitParm failed");
   InitModel();
   InitUtil();
   InitDict();
   InitNet(); 
   ...
   while (NextArg() == SWITCHARG) {
      s = GetSwtArg();
      if (strlen(s) != 1) 
         HError (4019, "HLConf: Bad switch %s; must be single letter",s);
      switch (s[0]){
      case 'L':
         if (NextArg() != STRINGARG)
            HError (4019,"HLConf: Lattice file directory expected");
         latInDir = GetStrArg(); 
         break;
      case 'X':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Lattice filename extension expected");
         latInExt = GetStrArg(); 
         break;
      case 'i':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Output MLF file name expected");
         if (SaveToMasterfile (GetStrArg()) < SUCCESS)
            HError (4014, "HLConf: Cannot write to MLF");
         break;

      case 'I':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Input MLF file name expected");
      ...
      case 'G':
         if (NextArg() != STRINGARG)
            HError (3219, "HLConf: Input Label File format expected");
         if((ifmt = Str2Format (GetStrArg ())) == ALIEN)
            HError (-3289, "HLConf: Warning ALIEN Input file format set");
         break;
      case 'P':
         if (NextArg() != STRINGARG)
            HError (3219, "HLConf: Output Label File format expected");
         if((ofmt = Str2Format (GetStrArg ())) == ALIEN)
            HError (-3289, "HLConf: Warning ALIEN Label output file format set");
         break;
      
      case 'l':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Output Label file directory expected");
         labOutDir = GetStrArg(); 
         break;
      case 'o':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Output label format expected");
         labOutForm = GetStrArg(); 
         break;
      case 'y':
         if (NextArg() != STRINGARG)
            HError (4019, "HLConf: Output label file extension expected");
         labOutExt = GetStrArg(); break;
      
      case 'p':
         if (NextArg() != FLOATARG)
            HError (4019, "HLConf: word insertion penalty expected");
         wordPen = GetChkedFlt (-1000.0, 1000.0, s); 
         break;
      case 's':
         if (NextArg() != FLOATARG)
            HError (4019, "HLConf:  grammar scale factor expected");
         lmScale = GetChkedFlt (0.0, 1000.0, s); 
         break;
      case 'r':
         if (NextArg() != FLOATARG)
            HError (4019, "HLConf:  pronunciation scale factor expected");
         prScale = GetChkedFlt (0.0, 1000.0, s); 
         break;
      case 'a':
         if (NextArg() != FLOATARG)
            HError (4019, "HLConf:  acoustic scale factor expected");
      ...
      case 'T':
         ...
         break;
      default:
         HError (4019, "HLConf: Unknown switch %s",s);
      }
   }
   ...
   while (NumArgs() > 0) {
      if (NextArg() != STRINGARG)
         HError (4019, "HLConf: Transcription file name expected");
      latfn = GetStrArg();
      if (latFileMask) {
      ...
      if (labOFileMask) {
         if (!MaskMatch (labOFileMask, labfn_ou, latfn))
            HError(2319,"HLRescore: LABOFILEMASK %s has no match with segemnt %s", labOFileMask, latfn);
      }
      else
...
void ClampACLike (Lattice *lat)
{
   ...
   for (i = 0, la = lat->larcs; i < lat->na; ++i, ++la) {
      if (la->aclike > 0) {
         HError (-9999, "HLattice: aclike of arc is %f  setting to 0.0\n", la->aclike);  /*#### GE fix errorcode */
         la->aclike = 0.0;
      }
   }
}
...
void InitSimScore(void)
{
   ...
   if ((SimGrid = (Cell **)malloc((MAXNPHONES+1)*sizeof(Cell*))) == NULL)
      HError(2,"InitSimScore: Cannot create grid pointer array");
   for (i=0; i<=MAXNPHONES;i++)
      if ((SimGrid[i] = (Cell*) calloc(MAXNPHONES+1,sizeof(Cell))) == NULL)
         HError(2,"InitSimScore: Cannot create grid column %d",i);
...
void WriteConfnet (ConfNet *cn, char *fn)
{
   ...
   if (!SCF)
      HError (1, "cannot open scf file '%s'", fn);
      ...
      for (scw = sc1->arc; scw; scw = scw->next) {
         if (fprintf(SCF, "W=%-19s ", 
                     ReWriteString (scw->word->wordName->name, 
                                    NULL, ESCAPE_CHAR)) < 0)
            HError (9999, "WriteConfnet: write failed on scf file");
...
void ConfNetClusterFile (char *latfn_in, char *latfn_ou, char *labfn_ou)
{
   ...
   if (trans) {
      ...
      if (LSave (labOutfn, trans, ofmt) < SUCCESS)
         HError(9999, "ConfNetClusterFile: Cannot save file %s", labOutfn);
      if (trace & T_TRAN) {
