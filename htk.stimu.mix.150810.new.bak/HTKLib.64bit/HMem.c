...
/* cz277 - xform */
NVecHost *CreateNVecHost(MemHeap *x, int nlen) {
    NVecHost *v;
    size_t size;

    v = (NVecHost *) New(x, sizeof(NVecHost));
    v->vecLen = nlen;
    size = NVectorElemSize(nlen);
#ifdef MKL
    v->vecElems = (NFloat *) mkl_malloc(size, MKL_alignment);
...
/* cz277 - xform */
NMatHost *CreateNMatHost(MemHeap *x, int nrows, int ncols) {
    NMatHost *m;
    size_t size;
 
    m = (NMatHost *) New(x, sizeof(NMatHost));
    m->rowNum = nrows;
    m->colNum = ncols;
    size = NMatrixElemSize(nrows, ncols);
#ifdef MKL
...
/* cz277 - xform */
void CopyVector2NVecHost(Vector v1, NVecHost *v2) {
    int i, size;

    size = VectorSize(v1);
    if (v2->vecLen != size) {
        HError(9999, "CopyVector2NVecHost: Inconsistent vector lengths");
    }
    for (i = 0; i < size; ++i)
        v2->vecElems[i] = v1[i + 1];
}
...
/* cz277 - xform */
void CopyNVecHost2Vector(NVecHost *v1, Vector v2)
{
    int i, size;

    size = VectorSize(v2);
    if (v1->vecLen != size) {
        HError(9999, "CopyNVecHost2Vector: Inconsistent vector lengths");
    }
    for (i = 0; i < size; ++i)
        v2[i + 1] = v1->vecElems[i];
}
...
/* cz277 - xform */
void CopyMatrix2NMatHost(Matrix m1, NMatHost *m2)
{
    int i, j, nrows, ncols;
    ...
    ncols = NumCols(m1);
    if (nrows != m2->rowNum || ncols != m2->colNum) {
        HError(9999, "CopyMatrix2NMatHost: Inconsistent matrices dimensions");
    }
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
            m2->matElems[i * ncols + j] = m1[i + 1][j + 1];
...
/* cz277 - xform */
void CopyNMatHost2Matrix(NMatHost *m1, Matrix m2) {
    int i, j, nrows, ncols;

    nrows = NumRows(m2);
    ncols = NumCols(m2);
    if (nrows != m1->rowNum || ncols != m1->colNum) {
        HError(9999, "CopyNMatHost2Matrix: Inconsistent matrices dimensions");
    }
    for (i = 0; i < nrows; ++i)
        for (j = 0; j < ncols; ++j)
            m2[i + 1][j + 1] = m1->matElems[i * ncols + j];
