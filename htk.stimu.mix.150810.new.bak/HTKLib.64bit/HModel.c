...
void InitANNSet(HMMSet *hset) {
    ...
    for (i = 1; i < MAXNUMRPLNMAT; ++i) {
        hset->annSet->rplNMatInfo[i] = NULL;
    }
    for (i = 1; i < MAXNUMRPLNVEC; ++i) {
        hset->annSet->rplNVecInfo[i] = NULL;
    }
}
    ...
    while (annInfo != NULL) {
        ...
        for (i = 0; i < annDef->layerNum; ++i) {
            ...
            if (layerElem->trainInfo != NULL) {
                ...
                /* cz277 - pact */
                for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {	/* no control/hyper parameters */
                    if (layerElem->trainInfo->updtInfo->actParmVec[j] != NULL)
                        FreeNVector(hset->hmem, layerElem->trainInfo->updtInfo->actParmVec[j]);
                }
                /* remove gradInfo */
                ...
                /* cz277 - pact */
                for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {	/* no control/hyper parameters */
                    if (layerElem->trainInfo->gradInfo->actParmVec[j] != NULL)
                        FreeNVector(hset->hmem, layerElem->trainInfo->gradInfo->actParmVec[j]);
                }
                if (layerElem->trainInfo->gradInfo->actCacheMats != NULL) {
                    n = IntVecSize(layerElem->drvCtx);
                    for (j = 1; j <= n; ++j)
                        FreeNMatrix(hset->hmem, layerElem->trainInfo->gradInfo->actCacheMats[j]);
                }
                /* remove nlrInfo */
                if (layerElem->trainInfo->nlrInfo != NULL) {
                    ...
                    /* cz277 - pact */
                    for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {	/* no control/hyper parameters */
                        if (layerElem->trainInfo->nlrInfo->actParmVec[j] != NULL)
                            FreeNVector(hset->hmem, layerElem->trainInfo->nlrInfo->actParmVec[j]);
                    }
                }
                /* remove ssgInfo */
                if (layerElem->trainInfo->ssgInfo != NULL) {
                    ...
                    /* cz277 - pact */
                    for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {	/* no control/hyper parameters */
                        if (layerElem->trainInfo->ssgInfo->actParmVec[j] != NULL)
                            FreeNVector(hset->hmem, layerElem->trainInfo->ssgInfo->actParmVec[j]);
                    }
                }
            ...
            /* cz277 - pact */
            for (j = 0; j < MAXNACTFUNPARMVEC; ++j) {
                if (layerElem->actFunInfo.actParmVec[j] != NULL)
                    FreeNVector(hset->hmem, layerElem->actFunInfo.actParmVec[j]);
            }
        }
...
/* cz277 - pact */
/* type can be u(pdtInfo), g(rad)Info, n(lr)Info, or s(sg)Info */
void InitTrainInfoActFun(HMMSet *hset, LELink layerElem, LILink layerInfo, Boolean clearOrNot, char type) {
    int i, n;

    /* for other parameter vectors */
    memset(layerInfo->actParmVec, 0, MAXNACTFUNPARMVEC * sizeof(NVector *));
    for (i = 1; i < MAXNACTFUNPARMVEC; ++i) {	/* no control/hyper parameter for train info */
        if (layerElem->actFunInfo.actParmVec[i] != NULL) {
            layerInfo->actParmVec[i] = CreateNVector(hset->hmem, layerElem->actFunInfo.actParmVec[i]->vecLen);
        }
    }
    /* initialise the activation value cache */
    if (type == 'g') {
        if (CacheActMatrixOrNot(layerElem->actFunInfo.actFunKind) == TRUE) {
            n = IntVecSize(layerElem->drvCtx);
            layerInfo->actCacheMats = (NMatrix **) New(hset->hmem, sizeof(NMatrix *) * (n + 1));
            for (i = 1; i <= n; ++i)
                layerInfo->actCacheMats[i] = CreateNMatrix(hset->hmem, layerElem->nodeNum, GetNBatchSamples());
        }
    }
    /* clear the vectors or not */
    if (clearOrNot == TRUE) {
        for (i = 1; i < MAXNACTFUNPARMVEC; ++i) {	/* no control/hyper parameter for train info */
            if (layerInfo->actParmVec[i] != NULL)
                ClearNVector(layerInfo->actParmVec[i]);
        }
    }
}
...
void InitTrainInfo(HMMSet *hset, Boolean initLabMats, Boolean initLRInfo, Boolean initSSGInfo, Boolean initStruct) {
    ...
    while (annInfo != NULL) {
        ...
        for (i = 0; i < annDef->layerNum; ++i) {
            layerElem = annDef->layerList[i];
	    /*n = IntVecSize(layerElem->drvCtx);*/	/* cz277 - many */
            if (layerElem->trainInfo == NULL) {
                ...
                memset(layerElem->trainInfo->updtInfo, 0, sizeof(LayerInfo));
                /* set lrInfo */
                layerElem->trainInfo->nlrInfo = NULL;
                /* set gradInfo */
                layerElem->trainInfo->gradInfo = (LILink) New(hset->hmem, sizeof(LayerInfo));
                memset(layerElem->trainInfo->gradInfo, 0, sizeof(LayerInfo));
                /* set ssgInfo */
                layerElem->trainInfo->ssgInfo = NULL;
                /* set dxFeaMat */
                /* layerElem->trainInfo->dxFeaMat = CreateNMatrix(hset->hmem, GetNBatchSamples(), layerElem->inputDim);*/	/* cz277 - many */
		...
		for (j = 1; j <= n; ++j) {
		    ...
		    layerElem->trainInfo->dyFeaMats[j] = NULL;
                ...
                layerElem->trainInfo->drvCnt = NULL;
            ...
            if (initLRInfo && layerElem->trainInfo->nlrInfo == NULL) {
                ...
                memset(layerElem->trainInfo->nlrInfo, 0, sizeof(LayerInfo));
            }
            ...
            if (initSSGInfo && layerElem->trainInfo->ssgInfo == NULL) {
                ...
                memset(layerElem->trainInfo->ssgInfo, 0, sizeof(LayerInfo));
#ifdef CUDA
            ...
            if (initStruct) {
                ...
                if (layerElem->trainInfo->updtInfo->wghtMat == NULL) {
                    InitTrainInfoActFun(hset, layerElem, layerElem->trainInfo->updtInfo, TRUE, 'u');	/* cz277 - pact */
		    ...
		    ClearNVector(layerElem->trainInfo->updtInfo->biasVec);
                ...
                if (layerElem->trainInfo->gradInfo->wghtMat == NULL) {
                    InitTrainInfoActFun(hset, layerElem, layerElem->trainInfo->gradInfo, FALSE, 'g');     /* cz277 - pact */
                ...
                if (initLRInfo && layerElem->trainInfo->nlrInfo->wghtMat == NULL) {
                    InitTrainInfoActFun(hset, layerElem, layerElem->trainInfo->nlrInfo, FALSE, 'n');     /* cz277 - pact */
                ...
                if (initSSGInfo && layerElem->trainInfo->ssgInfo->wghtMat == NULL) {
                    InitTrainInfoActFun(hset, layerElem, layerElem->trainInfo->ssgInfo, TRUE, 's');     /* cz277 - pact */
...
typedef enum {   /* Only a character big !! */
   ...
   RPLIDX, MATRIX, VECTOR,      /* cz277 - xform */
...
static struct {
   ...
   { { "BEGINHMM", BEGINHMM }, { "USE", USEMAC }, 
     ...
     {"OPERAND", OPERAND}, {"ACTIVATION", ACTIVATION}, {"INPUTFEA", INPUTFEA}, {"WEIGHT", WEIGHT}, {"ROWSHARE", ROWSHARE}, {"UPDATE", UPDATE}, {"LEARNRATE", LEARNRATE}, {"SQUAREGRAD", SQUAREGRAD}, {"ACTPARAMS", ACTPARAMS}, 
     {"RPLIDX", RPLIDX}, {"MATRIX", MATRIX}, {"VECTOR", VECTOR}, /* cz277 - xform */
     {"BEGINANN", BEGINANN}, {"ANNKIND", ANNKIND}, {"NUMLAYERS", NUMLAYERS}, {"LAYER", LAYER}, {"ENDANN", ENDANN}, 
...
static ReturnStatus GetToken(Source *src, Token *tok)
{
    ...
    if (c == '~') {                    /* If macro sym return immediately */
        c = tolower(GetCh(src));
        if (c != '1' && c != '2' &&	/* cz277 - xform */
            c != 's' && c != 'm' && c != 'u' && c != 'x' && c != 'd' && c != 'c' &&
...
ActFunKind Str2ActFunKind(char *str)
{
    ...
    if (!strcmp(str, "AFFINE"))
        ...
        HError(9999, "Unknown Activation Function Kind %s", str);
    return afkind;
}
...
static FeaMix *GetDefaultInpFeaMix(HMMSet *hset, int streamIdx)
{
    ...
    if (i == hset->nInp[streamIdx]) {   /* register a new FeaElem */
        ...
        /*feaElem->feaMat = CreateNMatrix(hset->hmem, GetNBatchSamples(), feaElem->extDim);*/	/* cz277 - many */
        feaElem->feaMats = NULL;	/* cz277 - many */
        feaElem->ctxPool = NULL;        /* cz277 - many */
        feaElem->augFeaIdx = 0;
        feaElem->feaSrc = NULL;
        feaElem->hisLen = 0;
        feaElem->hisMat = NULL;
    ...
    /* set mixMat */
    /*feaMix->mixMat = feaMix->feaList[0]->feaMat;*/	/* cz277 - many */
    feaMix->mixMats = NULL;     /* cz277 - many */
    feaMix->ctxPool = NULL;     /* cz277 - many */
    /* add a new macro */
    ...
    if (tok->sym == MACRO && tok->macroType == 'e') {   /* a reference to a pre-defined feature MACRO */
        ...
        for (i = 0; i < feaMix->elemNum; ++i) {    /* load each feature component */
            ...
            if (tok->sym == EXPAND) {
                ...
                feaElem->ctxMap = CreateIntVec(&gcheap, intVals[0]);
		    ...
		    /* cz277 - xform */
                    /*for (j = 1; j <= feaElem->ctxMap[0]; ++j) {
                        if (feaElem->ctxMap[j] < feaElem->hisLen) {
...
static NMatrix *GetNMatrix(HMMSet *hset, Source *src, Token *tok)
{
    ...
    /* cz277 - xform */
    /* if replaceable matrix */
    if (tok->sym == RPLIDX) {
        /* replaceable part index */
        if (!ReadInt(src, intVals, 1, tok->binForm)) {
            HMError(src, "Index of replaceable part expected");
            return NULL;
        }
        wghtMat->rplIdx = intVals[0];
        LoadInRplNMat(hset->annSet, wghtMat);
        /* get the next token */
        if (GetToken(src, tok) < SUCCESS) {
            HMError(src, "GetToken failed");
            return NULL;
        ...
        hook = (size_t *) New(hset->hmem, sizeof(size_t));
        *hook = 0;
	wghtMat->hook = (Ptr) hook;
    }
...
/* cz277 - form */
static NMatHost *GetNMatHost(HMMSet *hset, Source *src, Token *tok) {
    Matrix floatMat;
    int intVals[2];
    NMatHost *hostMat;
    ...
    if (tok->sym == MACRO && tok->macroType == '1') {
        hostMat = (NMatHost *) GetStructure(hset, src, '1');
        if (hostMat == NULL) {
            HMError(src, "GetStructure failed");
            return NULL;
        }
        ...
        hostMat = CreateNMatHost(hset->hmem, intVals[0], intVals[1]);
        CopyMatrix2NMatHost(floatMat, hostMat);
        FreeMatrix(&gstack, floatMat);
...
static NVector *GetNVector(HMMSet *hset, Source *src, Token *tok)
{
    ...
    if (tok->sym == RPLIDX) {
        ...
        biasVec->rplIdx = intVal;
        LoadInRplNVec(hset->annSet, biasVec);
        /* get the next token */
        if (GetToken(src, tok) < SUCCESS) {
            HMError(src, "GetToken failed");
            return NULL;
...
/* cz277 - xform */
static NVecHost *GetNVecHost(HMMSet *hset, Source *src, Token *tok)
{
    NVecHost *hostVec;
    int intVal;
    Vector floatVec;
    ...
    if (tok->sym == MACRO && tok->macroType == '2') {
        hostVec = (NVecHost *) GetStructure(hset, src, '2');
        if (hostVec == NULL) {
            HMError(src, "GetStructure failed");
            return NULL;
        }
        ...
        /* size */
        if (! ReadInt(src, &intVal, 1, tok->binForm)) {
            HMError(src, "Host vector size expected");
            return NULL;
        }
        floatVec = CreateVector(&gstack, intVal);
        if (!ReadVector(src, floatVec, tok->binForm)) {
            HMError(src, "Read host vector failed");
            return NULL;
        }
        hostVec = CreateNVecHost(hset->hmem, intVal);
        CopyVector2NVecHost(floatVec, hostVec);
        FreeVector(&gstack, floatVec);
        /* read next symbol */
        if (GetToken(src, tok) < SUCCESS) {
            HMError(src, "GetToken failed");
...
LELink GenBlankLayer(MemHeap *heap) {
    ...
    layerElem->xFeaMats = NULL;	/* cz277 - many */
    layerElem->yFeaMats = NULL;	/* cz277 - many */
    /*layerElem->expInput = FALSE;*/
    layerElem->roleKind = HIDRK;
    /*layerElem->trainInfo->updtFlag = ACTFUNUK | BIASUK | WEIGHTUK;*/
    layerElem->drvCtx = NULL;	/* cz277 - many */
    return layerElem;
}

/* cz277 - pact */
static Boolean GetActFunParams(Source *src, Token *tok, HMMSet *hset, LELink layerElem) {
    int i, var; 
    ...
    var = GetActFunNParmVector(layerElem);
    /* load hyper/control parameters */
    if (var < 0) {
        if (tok->sym != ACTPARAMS) {
            HMError(src, "<ACTPARAMS> expected for parameterised activition function");
            return FALSE;
        }
        layerElem->actFunInfo.actParmVec[0] = GetNVector(hset, src, tok);
        if (var == ACTFUNPARMNOTLOAD) {
            var = GetActFunNParmVector(layerElem);	/* control/hyper parameters */
            if (var == ACTFUNPARMNOTLOAD) {
                HMError(src, "Control/hyper paramter vector should have been loaded");
                return FALSE;
            }
        }
        var *= -1;
    ...
    for (i = 1; i <= var; ++i) {
        ...
        layerElem->actFunInfo.actParmVec[i] = GetNVector(hset, src, tok);
    }
    ...
    if (tok->sym == MACRO && tok->macroType == 'k') {
        ...
        */
        ...
        if (tok->sym == ACTIVATION) {
            ...
            layerElem->actFunInfo.actFunKind = Str2ActFunKind(buf);
            if (GetActFunParams(src, tok, hset, layerElem) < SUCCESS) {       /* cz277 - pact */
                return NULL;
            }
        }
        if (tok->sym == INPUTFEA) {
...
static ReturnStatus GetANNTrainInfo(HMMSet *hset, Source *src, Token *tok, ADLink annDef)
{
    ...
    for (i = 0; i < annDef->layerNum; ++i) {
        /* cz277 - pact */
        memset(actParmVec, 0, MAXNACTFUNPARMVEC * sizeof(NVector *));
	...
	/* <ACTPARAMS> */
        /* cz277 - pact */
        for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {
            if (tok->sym == ACTPARAMS) {
                actParmVec[j] = GetNVector(hset, src, tok);
            }
        } 
        ...
        switch (sym) {
            case UPDATE:
                curInfo = layerElem->trainInfo->updtInfo;
                break;
            case LEARNRATE:
                if (layerElem->trainInfo->nlrInfo == NULL) {
                    InitTrainInfo(hset, FALSE, TRUE, FALSE, FALSE);
                }
                curInfo = layerElem->trainInfo->nlrInfo;
                break;
            case SQUAREGRAD:
                if (layerElem->trainInfo->ssgInfo == NULL) {
                    InitTrainInfo(hset, FALSE, FALSE, TRUE, FALSE);
                }
                curInfo = layerElem->trainInfo->ssgInfo;
                break;
            default:
                HMError(src, "Unknown symbol type");
                break;
        }
        /* cz277 - pact */
        /* assign the parameters */
        curInfo->wghtMat = wghtMat;
        curInfo->biasVec = biasVec;
        /* no matter actParmVec[j] is NULL or not */
        for (j = 0; j < MAXNACTFUNPARMVEC; ++j) {
            curInfo->actParmVec[j] = actParmVec[j];
        }
    }
...
static void PutNVector(HMMSet *hset, FILE *f, NVector *srcVec, Boolean binary) 
{
    ...
    /* rplidx */
    if (srcVec->rplIdx > 0) {
        PutSymbol(f, RPLIDX, binary);
        intTmp = srcVec->rplIdx;
        WriteInt(f, &intTmp, 1, binary);
        if (!binary)
            fprintf(f, "\n");
    }
...
static void PutNVecHost(HMMSet *hset, FILE *f, MLink q, NVecHost *hostVec, Boolean inMacro, Boolean binary)
{
    Vector floatVec;
    int intTmp;

    if (inMacro)
        PutMacroHdr(hset, f, q, '2', hostVec, binary);
    PutSymbol(f, VECTOR, binary);
    /* write size */
    intTmp = (int) hostVec->vecLen;
    WriteInt(f, &intTmp, 1, binary);
    if (!binary)
        fprintf(f, "\n");
    /* convert the vector */
    floatVec = CreateVector(&gstack, hostVec->vecLen);
    CopyNVecHost2Vector(hostVec, floatVec);
    /* write the vector */
    WriteVector(f, floatVec, binary);
    /* rplidx */
    PutSymbol(f, RPLIDX, binary);
    intTmp = hostVec->rplIdx;
    WriteInt(f, &intTmp, 1, binary);
    if (!binary)
        fprintf(f, "\n");
    /* free the temp vector */
...
static void PutNMatHost(HMMSet *hset, FILE *f, MLink q, NMatHost *hostMat, Boolean inMacro, Boolean binary)
{
    Matrix floatMat;
    int intTmp;

    if (inMacro)
        PutMacroHdr(hset, f, q, '1', hostMat, binary);
    PutSymbol(f, MATRIX, binary);
    /* write rows and cols */
    intTmp = hostMat->rowNum;
    WriteInt(f, &intTmp, 1, binary);
    intTmp = hostMat->colNum;
    WriteInt(f, &intTmp, 1, binary);
    if (!binary)
        fprintf(f, "\n");
    /* convert the matrix */
    floatMat = CreateMatrix(&gstack, hostMat->rowNum, hostMat->colNum);
    CopyNMatHost2Matrix(hostMat, floatMat);
    /* write the matrix */
    WriteMatrix(f, floatMat, binary);
    /* rplidx */
    PutSymbol(f, RPLIDX, binary);
    intTmp = hostMat->rplIdx;
    WriteInt(f, &intTmp, 1, binary);
    if (!binary)
        fprintf(f, "\n");
    /* free the temp matrix */
...
static void PutLayerElem(HMMSet *hset, FILE *f, MLink q, LELink layerElem, Boolean inMacro, Boolean binary)
{
    ...
    if (layerElem->nUse == 0 || inMacro) {
        ...
        if (!binary)
            fprintf(f, " ");
        WriteString(f, ActFunKind2Str(layerElem->actFunInfo.actFunKind, buf), DBL_QUOTE);
        if (!binary)
            fprintf(f, "\n");
        /* cz277 - pact */
        /* <ACTPARAMS> (optional), activation function parameters */
        for (i = 0; i < MAXNACTFUNPARMVEC; ++i) {
            if (layerElem->actFunInfo.actParmVec[i] != NULL) {
                PutSymbol(f, ACTPARAMS, binary);
                PutNVector(hset, f, layerElem->actFunInfo.actParmVec[i], binary);
            }
        }
...
static void PutANNTrainInfo(HMMSet *hset, FILE *f, MLink q, ADLink annDef, Symbol sym, Boolean binary)
{
    ...
    for (i = 0; i < annDef->layerNum; ++i) {
        ...
        /* cz277 - pact */
        for (j = 1; j < MAXNACTFUNPARMVEC; ++j) {	/* no control/hyper parameters for train info */
            if (layerInfo->actParmVec[j] != NULL) {
                PutSymbol(f, ACTPARAMS, binary);
                PutNVector(hset, f, layerInfo->actParmVec[j], binary);
            }
        }
...
static ReturnStatus LoadAllMacros(HMMSet *hset, char *fname, short fidx)
{
    ...
    Ptr structure;
    MLink m;
    char type, buf[MAXSTRLEN], buf1[MAXSTRLEN];
    LabId id;
    HLink dhmm;
    HMMSet dset;
        ...
        if (type == 'o') {    /* load an options macro */
            ...
            if (type == 'h') {    /* load a HMM definition */
                ...
                switch (type) {
                    /* cz277 - xform */
                    case '1': 
                        hostMat = GetNMatHost(hset, &src, &tok);	
                        strcpy(buf1, buf);
                        MakeRplNMatName(buf1, hostMat->rplIdx, buf); 
                        id = GetLabId(buf, TRUE);
                        structure = (Ptr) hostMat;
                        break;
		    case '2': 
                        hostVec = GetNVecHost(hset, &src, &tok);	
                        strcpy(buf1, buf);
                        MakeRplNVecName(buf1, hostVec->rplIdx, buf);
                        id = GetLabId(buf, TRUE);
                        structure = (Ptr) hostVec;
                        break;
    ...
    for (i = 0; i < hset->annSet->annNum; ++i) {
	    ...
	    /* cz277 - 1007 */
            ...
            if (curAD->layerList[j]->feaMix->mixDim != curAD->layerList[j]->inputDim) {
                printf("STIMU [warn]: CheckANNConsistency: Input FeaMix dim %d does not match the input dim %d of the LayerElem\n", 
                        curAD->layerList[j]->feaMix->mixDim, curAD->layerList[j]->inputDim);
                //HError(9999, "CheckANNConsistency: Input FeaMix dim %d does not match the input dim %d of the LayerElem", curAD->layerList[j]->feaMix->mixDim, curAD->layerList[j]->inputDim);
            }
        }
        /* fetch the next ANNInfo */
        curAI = curAI->next;
...
void FindANNCycles(HMMSet *hset) {
    ...
    /* post processing */
    if (cycCnt > 0)
        HError(9999, "FindANNCycles: %d cycles were detected", cycCnt);
    /* reset status */
    curAI = hset->annSet->defsHead;
    while (curAI != NULL) {
        curAD = curAI->annDef;
...
/* cz277 - many */
static IntVec MergeSortedContext(IntVec lhCtx, IntVec rhCtx) {
                ...
                for (k = 1; k <= IntVecSize(layerElem->drvCtx); ++k) {
                    ...
                    if (feaElem->inputKind == ANNFEAIK) {
                        ...
                        if (srcElem->drvCtx == NULL) {
                            ...
                            resVec = MergeSortedContext(srcElem->drvCtx, shiftVec);    
...
/*chaopig*/
#ifdef CUDA
void ShowDxDyMats(HMMSet *hset) {
    int c, i, j, k, l, n, m, p, curOff;
    AILink curAI;
    ADLink curAD;
    LELink layerElem, srcElem;
    FELink errElem, feaElem;
    ...
    while (curAI != NULL) {
        ...
        for (i = 0; i < curAD->layerNum; ++i) {
            layerElem = curAD->layerList[i];
            n = IntVecSize(layerElem->drvCtx);
            printf("layer i = %d, layerElem = %p, n = %d\n", i, layerElem, n);
            for (j = 1; j <= n; ++j) {
                if (layerElem->trainInfo->dxFeaMats[j] != NULL)
                    printf("\tj = %d, drvCtx[j] = %d, dxFeaMats[j] = %p, dxFeaMats[j]->matElems = %p, dxFeaMats[j]->devElems = %p\n", j, layerElem->drvCtx[j], layerElem->trainInfo->dxFeaMats[j], layerElem->trainInfo->dxFeaMats[j]->matElems, layerElem->trainInfo->dxFeaMats[j]->devElems);
                if (layerElem->trainInfo->dyFeaMats[j] != NULL)
                    printf("\tj = %d, drvCtx[j] = %d, dyFeaMats[j] = %p, dyFeaMats[j]->matElems = %p, dyFeaMats[j]->devElems = %p\n", j, layerElem->drvCtx[j], layerElem->trainInfo->dyFeaMats[j], layerElem->trainInfo->dyFeaMats[j]->matElems, layerElem->trainInfo->dyFeaMats[j]->devElems);
            }
        }
        curAI = curAI->next;
    }
    printf("OUT ShowDxDyMats\n\n\n\n");
}
#endif
...
void InitErrMix(HMMSet *hset) {
    ...
    while (curAI != NULL) {
		...
		/* cz277 - many */
                n = IntVecSize(layerElem->drvCtx);	
                /*errMix->ctxPool = CreateIntVec(hset->hmem, n);*/
                errMix->ctxPool = CreateIntVec(&gcheap, n);
                CopyIntVec(layerElem->drvCtx, errMix->ctxPool);	
		/*errMix->mixMats = (NMatrix **) New(hset->hmem, sizeof(NMatrix *) * (n + 1));*/
		    ...
		    errElem = layerElem->errMix->feaList[j];
                    ...
                    errElem->ctxMap[1] = 0;*/	/* cz277 - many */
                    errElem->ctxMap = NULL;	/* cz277 - many */
                    errElem->ctxPool = NULL;	/* cz277 - many */
                    errElem->inputKind = ERRFEAIK;
                    errElem->feaSrc = NULL;
                    errElem->feaMats = NULL;
                    errElem->dimOff = -1;
                    errElem->extDim = -1;/*errElem->feaDim * errElem->ctxMap[0];*/
                    errElem->srcDim = -1;
                    errElem->augFeaIdx = 0;
                    errElem->nUse = 0;
                }
                        ...
                        if (errElem->feaDim == errElem->srcDim) {
                            ...
                            for (j = 1; j <= n; ++j) {
                                if (layerElem->errMix->ctxPool[j] != errElem->ctxPool[j])
                                    HError(9999, "InitErrMix: Unmatched context pool when sharing dyFeaMats");
...
static void SaveMacros(FILE *f, HMMSet *hset, short fidx, Boolean binary)
{
   ...
   if (saveRegTree) { /* then the regression classes */
    ...
    for (h = 0; h < MACHASHSIZE; ++h)
        for (m = hset->mtab[h]; m != NULL; m = m->next)
            if (m->fidx == fidx && m->type == '1')
                PutNMatHost(hset, f, m, (NMatHost *) m->structure, TRUE, binary);
    for (h = 0; h < MACHASHSIZE; ++h)
        for (m = hset->mtab[h]; m != NULL; m = m->next)
            if (m->fidx == fidx && m->type == '2')
                PutNVecHost(hset, f, m, (NVecHost *) m->structure, TRUE, binary);
}
...
/* cz277 - xform */
NMatHost *LoadOneRplNMat(HMMSet *hset, char *path, char *fname, char *macroname) {
    Source src;
    Token tok;
    LabId id;
    Ptr structure;
    ...
    /* First see whether the macro exists */
    id = GetLabId(macroname, FALSE);
    if (id == NULL || (m = FindMacroName(hset, '1', id)) == NULL) {
        MakeFN(fname, path, NULL, absfn);
        if (InitScanner(absfn, &src, &tok, HMMDefFilter) < SUCCESS) 
            HError(9999, "LoadOneRplNMat: Fail to load %s", absfn);
        if (GetToken(&src, &tok) < SUCCESS)
            HError(9999, "LoadOneRplNMat: Fail to acquire the next term");
        /* process the macro or defination */
        if (tok.sym == MACRO && tok.macroType == '1') {
            if (! ReadString(&src, buf)) 
                HError(9999, "LoadOneRplNMat: Macro name expected");
            hostMat = GetNMatHost(hset, &src, &tok);
            MakeRplNMatName(buf, hostMat->rplIdx, namebuf);
            if (strcmp(namebuf, macroname) != 0)
                HError(9999, "LoadOneRplNMat: Macro name unmatched %s vs. %s", macroname, namebuf);
            if (GetToken(&src, &tok) < SUCCESS)
                HError(9999, "LoadOneRplNMat: Fail to acquire the next term"); 
        }
        else {
            if (tok.sym != MATRIX) 
                HError(9999, "LoadOneRplNMat: <MATRIX> key word expected");
            hostMat = GetNMatHost(hset, &src, &tok); 
            id = GetLabId(macroname, TRUE);
            NewMacro(hset, fidx, '1', id, (Ptr) hostMat);
        }
        TermScanner(&src);     
    }
    else {
        hostMat = (NMatHost *) m->structure;
    }
...
/* cz277 - xform */
NVecHost *LoadOneRplNVec(HMMSet *hset, char *path, char *fname, char *macroname) {
    Source src;
    Token tok;
    LabId id;
    Ptr structure;
    ...
    /* First see whether the macro exists */
    id = GetLabId(macroname, FALSE);
    if (id == NULL || (m = FindMacroName(hset, '2', id)) == NULL) {
        MakeFN(fname, path, NULL, absfn);
        if (InitScanner(absfn, &src, &tok, HMMDefFilter) < SUCCESS)
            HError(9999, "LoadOneRplNVec: Fail to load %s", absfn);
        if (GetToken(&src, &tok) < SUCCESS)
            HError(9999, "LoadOneRplNVec: Fail to acquire the next term");
        /* process the macro or defination */
        if (tok.sym == MACRO && tok.macroType == '2') {
            if (! ReadString(&src, buf)) 
                HError(9999, "LoadOneRplNVec: Macro name expected");
            hostVec = GetNVecHost(hset, &src, &tok);
            MakeRplNVecName(buf, hostVec->rplIdx, namebuf);
            if (strcmp(namebuf, macroname) != 0)
                HError(9999, "LoadOneRplNVec: Macro name unmatched %s vs. %s", macroname, namebuf);
            if (GetToken(&src, &tok) < SUCCESS)
                HError(9999, "LoadOneRplNVec: Fail to acquire the next term"); 
        }
        else {
            if (tok.sym != VECTOR) 
                HError(9999, "LoadOneRplNVec: <VECTOR> key word expected");
            hostVec = GetNVecHost(hset, &src, &tok);
            id = GetLabId(macroname, TRUE);   
            NewMacro(hset, fidx, '2', id, (Ptr) hostVec);
        }
        TermScanner(&src);
    }
    else {
        hostVec = (NVecHost *) m->structure;
    }
...
/* cz277 - xforms */
void SaveOneRplNMat(HMMSet *hset, NMatHost *hostMat, char *fname, Boolean binary) {
    FILE *f;
    Boolean isPipe;

    if ((f = FOpen(fname, HMMDefOFilter, &isPipe)) == NULL)
        HError(9999, "SaveOneRplNMat: Cannot create output file %s", fname);
    if (trace & T_MAC)
        printf("HModel: Saving replaceable matrix to %s", fname);
    PutNMatHost(hset, f, NULL, hostMat, TRUE, binary);
}

/* cz277 - xforms */
void SaveOneRplNVec(HMMSet *hset, NVecHost *hostVec, char *fname, Boolean binary) {
    FILE *f;
    Boolean isPipe;

    if ((f = FOpen(fname, HMMDefOFilter, &isPipe)) == NULL)
        HError(9999, "SaveOneRplNVec: Cannot create output file %s", fname);
    if (trace & T_MAC)
        printf("HModel: Saving replaceable vector to %s", fname);
    PutNVecHost(hset, f, NULL, hostVec, TRUE, binary);
}
