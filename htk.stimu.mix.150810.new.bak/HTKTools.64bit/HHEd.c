...
static void InitActParmVecs(HMMSet *hset, LELink layerElem) {
    ...
    if (var < 0) {
        ...
        case HERMITEAF:
            layerElem->actFunInfo.actParmVec[0] = CreateNVector(hset->hmem, 1);
            layerElem->actFunInfo.actParmVec[0]->vecElems[0] = var;
            break;
        default:
            break;
        }
    ...
    for (i = 1; i <= var; ++i) {
        layerElem->actFunInfo.actParmVec[i] = CreateNVector(hset->hmem, lengths[i]);
        floatVal = ChkedFloat("Initial value for current parameter vector", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[i]);
    }

    /*switch (layerElem->actFunInfo.actFunKind) {
    case AMVNAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Affine function shift", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[1]);
        layerElem->actFunInfo.actParmVec[2] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Affine function scale", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[2]);
        break;
    case HERMITEAF:
        layerElem->actFunInfo.actParmVec[0] = CreateNVector(hset->hmem, 1);
        intVal = ChkedInt("Hermite Polynomial coefficient", 1, MAXNACTFUNPARMVEC);
        layerElem->actFunInfo.actParmVec[0]->vecElems[0] = intVal;
        for (i = 1; i <= intVal; ++i) {
            layerElem->actFunInfo.actParmVec[i] = CreateNVector(hset->hmem, layerElem->nodeNum);
            floatVal = ChkedFloat("Hermite polynomial coefficient initial value", -1.0 * FLOAT_MAX, FLOAT_MAX);    
            SetNVector(floatVal, layerElem->actFunInfo.actParmVec[i]);
        }
        break;
    case LHUCPLRELUAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(PLRELUNEGSCALE, layerElem->actFunInfo.actParmVec[1]);
        layerElem->actFunInfo.actParmVec[2] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(0.0, layerElem->actFunInfo.actParmVec[2]);
        break;
    case LHUCRELUAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(0.0, layerElem->actFunInfo.actParmVec[1]);
        break;
    case LHUCSIGMOIDAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(0.0, layerElem->actFunInfo.actParmVec[1]);
    ...
    case PASIGMOIDAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(1.0, layerElem->actFunInfo.actParmVec[1]);
        break;
    case PABSIGMOIDAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(1.0, layerElem->actFunInfo.actParmVec[1]);
        layerElem->actFunInfo.actParmVec[2] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(1.0, layerElem->actFunInfo.actParmVec[2]); 
        break;
    case PBSIGMOIDAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        SetNVector(1.0, layerElem->actFunInfo.actParmVec[1]);
    ...
    case PABCSIGMOIDAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Amplifier alpha", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[1]);
        layerElem->actFunInfo.actParmVec[2] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Scaling factor beta for the input", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[2]);
        layerElem->actFunInfo.actParmVec[3] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Offset for the input", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[3]);
        break;
    case PLRELUAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
        floatVal = ChkedFloat("Scaling factor for y <= 0", -1.0 * FLOAT_MAX, FLOAT_MAX);
        SetNVector(floatVal, layerElem->actFunInfo.actParmVec[1]);
        break;
    case POLYLINEARAF:
        layerElem->actFunInfo.actParmVec[1] = CreateNVector(hset->hmem, layerElem->nodeNum);
...
void InsertOneANNLayer(void) {
    ...
    if (strcmp(buf, "<ACTIVATION>") == 0) {
        ...
        layerElem->actFunInfo.actFunKind = Str2ActFunKind(buf);	/* cz277 - pact */
        InitActParmVecs(hset, layerElem);	/* cz277 - pact */
        ChkedAlpha("Next keyword", buf);
        /*if (layerElem->actFunInfo.actFunKind == RELUAF || layerElem->actFunInfo.actFunKind == SOFTRELUAF) {
...
void EraseOneANNLayer(void) {
    ...
    /* take action */
    actfunKind = layerElem->actFunInfo.actFunKind;
    ...
    RandANNLayer(layerElem, layerPos, randScale);
    layerElem->actFunInfo.actFunKind = actfunKind;
    /* saves the model */
    saveHMMSet = TRUE;
}
...
static FeaMix *CloneOneFeaMix(FeaMix *srcMix) {
    ...
    for (i = 0; i < srcMix->elemNum; ++i) {
        ...
        for (j = 1; j <= n; ++j) {
            /*dstElem->feaMats[j] = CreateNMatrix(hset->hmem, GetNBatchSamples(), dstElem->extDim);*/
...
/*#ifdef CUDA
    ...
    lowLayer->actFunInfo.actFunKind = LINEARAF;
...
void CopyANNLayerParms(void) {
    ...
    for (i = 0; i < opCnt; ++i) {
        ...
        if (strcmp(buf, "<COPY>") == 0 && auxset != NULL) {
            ...
            dstLayer = GetLayerElemByMacName("CopyANNLayerParms", NULL, hset, NULL, NULL);
            opCode = ChkedInt("Operational code", 0, INT_MAX);
            if ((opCode & BIASUK) != 0 && auxinp->xform->bias != NULL) {
                if (VectorSize(auxinp->xform->bias) != dstLayer->inputDim) {
                    HError(-1, "CopyANNLayerParms: Unequal bias vector lenghts");
                }
                CopyVector2NVector(auxinp->xform->bias, dstLayer->biasVec);
            }
            if ((opCode & WEIGHTUK) != 0) {
                if (NumRows(auxinp->xform->xform[1]) != dstLayer->nodeNum || NumCols(auxinp->xform->xform[1]) != dstLayer->inputDim) {
                    HError(-1, "CopyANNLayerParms: Unequal weight matrix dimensions");
                }
                CopyMatrix2NMatrix(auxinp->xform->xform[1], dstLayer->wghtMat);
            }
        }
        else {
...
/* ------------------- SI - Set replaceable index ------------------ */

/* cz277 - SI */
void SetANNRPLIndex(void) {
    char buf[MAXSTRLEN];
    LELink layerElem;
    int actIdx, rplIdx, newIdx;

    /* get the layer */
    ChkedAlpha("Target layer macro type", buf);
    if (strcmp(buf, "~n") == 0 || strcmp(buf, "~k") == 0) {
        layerElem = GetLayerElemByMacName("AddANNRPLIndex", buf, hset, NULL, NULL);
    }
    else {
        HError(9999, "AddANNRPLIndex: Unsupported target ANN layer macro");
    }
    /* load the target type */
    ChkedAlpha("Replaceable part type", buf);
    if (strcmp(buf, "<ACTIVATION>") == 0) {
        actIdx = ChkedInt("Activation function parameter index", 1, MAXNACTFUNPARMVEC);
        ChkedAlpha("<RPLIDX> keyword", buf);
        if (strcmp(buf, "<RPLIDX>") != 0) {
            HError(9999, "SetANNRPLIndex: <RPLIDX> keyword expected in SI command");
        }
        newIdx = ChkedInt("New replaceable part index", 1, MAXNUMRPLNVEC);
        if (layerElem->actFunInfo.actParmVec[actIdx] != NULL) {
            rplIdx = layerElem->actFunInfo.actParmVec[actIdx]->rplIdx;
            if (rplIdx != newIdx) {
                if (rplIdx > 0 && rplIdx < MAXNUMRPLNVEC) {
                    hset->annSet->rplNVecInfo[rplIdx] = NULL;
                }
                layerElem->actFunInfo.actParmVec[actIdx]->rplIdx = newIdx;
                LoadInRplNVec(hset->annSet, layerElem->actFunInfo.actParmVec[actIdx]);
            }
        }
    }
    else if (strcmp(buf, "<WEIGHT>") == 0) {
        rplIdx = layerElem->wghtMat->rplIdx;
        ChkedAlpha("<RPLIDX> keyword", buf);
        if (strcmp(buf, "<RPLIDX>") != 0) {
            HError(9999, "SetANNRPLIndex: <RPLIDX> keyword expected in SI command");
        }
        newIdx = ChkedInt("New replaceable part index", 1, MAXNUMRPLNMAT); 
        if (rplIdx != newIdx) {
            if (rplIdx > 0 && rplIdx < MAXNUMRPLNMAT) {
                hset->annSet->rplNMatInfo[rplIdx] = NULL;
            }
            layerElem->wghtMat->rplIdx = newIdx;
            LoadInRplNMat(hset->annSet, layerElem->wghtMat);
        }
    }
    else if (strcmp(buf, "<BIAS>") == 0) {
        rplIdx = layerElem->biasVec->rplIdx;
        ChkedAlpha("<RPLIDX> keyword", buf);
        if (strcmp(buf, "<RPLIDX>") != 0) {
            HError(9999, "SetANNRPLIndex: <RPLIDX> keyword expected in SI command");
        }
        ...
        if (rplIdx != newIdx) {
            ...
            layerElem->biasVec->rplIdx = newIdx;
            LoadInRplNVec(hset->annSet, layerElem->biasVec); 
        }
    }
    else {
        HError(9999, "SetANNRPLIndex: Unknown replaceable part type");
    }
}
...
static char *cmdmap[] = {"AT","RT","SS","CL","CO","JO","MU","TI","UF","NC",
                         ...
                         "AF", "IL", "CF", "CD", "EL", "CA", "EF", "LR", "CP", "DL", "CM", "RS", "CH", "SL", "SI", "" };
...
void DoEdit(char * editFn)
{
   ...
   for (;;) {
      ...
      case SI: SetANNRPLIndex(); break;				/* done */
      default: 
